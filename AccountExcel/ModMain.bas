Attribute VB_Name = "ModMain"
    ...
    Public IsRepeatCode As Boolean
    Public IsRepeatName As Boolean
...
For mm = 1 To NoOFDigitUserTrans
    If user_id < 9 And NoOFDigitUserTrans > 1 And NoOFDigitUserTrans > mm Then
        mFormatUser = mFormatUser & "0"
    ElseIf user_id > 9 And user_id < 100 And NoOFDigitUserTrans > 1 And NoOFDigitUserTrans > mm + 1 Then
        mFormatUser = mFormatUser & "0"
    ElseIf user_id > 99 And NoOFDigitUserTrans > 1 And NoOFDigitUserTrans > mm + 2 Then
        mFormatUser = mFormatUser & "0"
    End If
...
branchpadidng = BranchDigit - 1
    ...
    If IsSerialByUserTrans Then
            ...
            mWhere4 = "SUBSTRING(CAST(cast(NoteSerial1 AS BIGINT) AS VARCHAR(100)),2 , " & NoOFDigitUserTrans & ") = " & user_id
           mWhereUser = mWhereUser & " AND " & user_id & "  IN ("
           mWhereUser = mWhereUser & " SELECT UserID FROM DOUBLE_ENTREY_VOUCHERS AS dev WHERE dev.Notes_ID = Notes.NoteID)"
            mWhereUser2 = " And UserID = " & user_id
...
If IsSerialByUserTrans Then
    Voucher_coding = Voucher_codingByUser(my_branch, date1, Sanad_No, NoteType, departement_name, Transaction_Type, Prefix, StoreId, BillType, MosemID)
    Exit Function
End If
...
XL1:
    ElseIf numbering_type = 2 Then '„ ’· ”‰‘Â—ÌÊÌ
        ...
        If JLCodeBasedOnBranch = False Then
            ...
            If JLCodeBasedOnBranch = False Then
sql = sql & " and       (SUBSTRING(CAST(CAST(NoteSerial AS int) AS varchar(100)), 5, 2) = SUBSTRING(CONVERT(VARCHAR(10), NoteDate, 110), 1, 2))"
sql = sql & " and       (SUBSTRING(CAST(CAST(NoteSerial AS int) AS varchar(100)), 1, 4) = SUBSTRING(CONVERT(VARCHAR(10), NoteDate, 110), 7, 4))"
End If
        Else
...
For mm = 1 To NoOFDigitUserVouc
    If user_id < 9 And NoOFDigitUserVouc > 1 And NoOFDigitUserVouc > mm Then
        mFormatUser = mFormatUser & "0"
    ElseIf user_id > 9 And user_id < 100 And NoOFDigitUserVouc > 1 And NoOFDigitUserVouc > mm + 1 Then
        mFormatUser = mFormatUser & "0"
    ElseIf user_id > 99 And NoOFDigitUserVouc > 1 And NoOFDigitUserVouc > mm + 2 Then
        mFormatUser = mFormatUser & "0"
    End If
...
End If
 Dim mWhere4 As String
    ...
    If IsSerialByUserVouch Then
            ...
            mWhere4 = "SUBSTRING(CAST(cast(NoteSerial AS BIGINT) AS VARCHAR(100)),2 , " & NoOFDigitUserVouc & ") = " & user_id
...
'                            Optional BolLastAcc As Boolean = 0, Optional ByVal BranchID As Integer = 0)
'
