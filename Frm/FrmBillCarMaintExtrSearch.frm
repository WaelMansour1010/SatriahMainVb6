...
Begin VB.Form FrmBillCarMaintExtrSearch 
   BackColor       =   &H00E2E9E9&
   ...
   ClientTop       =   435
   ClientWidth     =   6780
   Icon            =   "FrmBillCarMaintExtrSearch.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   RightToLeft     =   -1  'True
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VSFlex8UCtl.VSFlexGrid VSFlexGrid1 
         ...
         FormatString    =   $"FrmBillCarMaintExtrSearch.frx":038A
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSFlex8UCtl.VSFlexGrid Fg 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   -1  'True
         FormatString    =   $"FrmBillCarMaintExtrSearch.frx":0469
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "FrmBillCarMaintExtrSearch"
...
Private Sub Cmd_Click(Index As Integer)
    ...
    Select Case Index
        ...
        Case 0
If IndTyp = 1 Or IndTyp = 2 Then
GetDataMaintenanceWork
Else
GetData
End If
...
Private Sub Fg_Click()
 ...
 FrmBillCarMaintExtra.Retrive (val(Me.FG.TextMatrix(Me.FG.Row, Me.FG.ColIndex("id"))))
...
End Sub
Public Sub GetDataMaintenanceWork()
 ...
 StrSQL = StrSQL & " From dbo.TblMaintenanceWork"
    ...
    If val(Me.TxtFrom.Text) <> 0 Then
        If BolBegine = True Then
            StrWhere = StrWhere & " ID >=" & val(Me.TxtFrom.Text) & ""
        Else
            BolBegine = True
            StrWhere = " Where ID >=" & val(Me.TxtFrom.Text) & ""
        End If
    End If
    ...
    If val(Me.txtto.Text) <> 0 Then
        If BolBegine = True Then
            StrWhere = StrWhere & " AND ID <=" & val(Me.txtto.Text) & ""
        Else
            BolBegine = True
            StrWhere = " Where ID <=" & val(Me.txtto.Text) & ""
        End If
    End If
...
Dim k As Integer
    If rs.BOF Or rs.EOF Then
        If SystemOptions.UserInterface = ArabicInterface Then
            Me.lbl(12).Caption = "‰ ÌÃ… «·»ÕÀ=’›—"
        ElseIf SystemOptions.UserInterface = EnglishInterface Then
            Me.lbl(12).Caption = "Search Results=0"
        End If
        ...
        With Me.VSFlexGrid1
            ...
            For I = .FixedRows To .Rows - 1
                .TextMatrix(I, .ColIndex("Serial")) = I
                 k = IIf(IsNull(rs("Ling").value), 0, rs("Ling").value)

                Select Case IIf(IsNull(rs("HDWM").value), 0, rs("HDWM").value)
                Case 1
                k = k * 24
                Case 2
                k = k * 24 * 7
                ...
                k = k * 24 * 30
                End Select
                .TextMatrix(I, .ColIndex("Ling")) = k
                .TextMatrix(I, .ColIndex("value")) = IIf(IsNull(rs("InitialPrice").value), 0, rs("InitialPrice").value)
...
' StrSQL = StrSQL + "      dbo.TblCardAuthorizationReform.Telephone, dbo.TblCardAuthorizationReform.PlateNo, dbo.TblUsers.UserName, dbo.TblUsers.UserID AS Expr1,"
'   StrSQL = StrSQL + "     dbo.TblCardAuthorizationReform.UserID"
' StrSQL = StrSQL + "         FROM         dbo.TblCardAuthorizationReform LEFT OUTER JOIN"
'  StrSQL = StrSQL + "       dbo.TblUsers ON dbo.TblCardAuthorizationReform.UserID = dbo.TblUsers.UserID"
StrSQL = " SELECT     ID, RecordDate, ClientName, Telephone, UserID, Posted, PlateNo, YearFact, OrderStatus, Accept, EndDate, Month_Day, Granty, DateStartG, DateEndG, CarMeter,"
StrSQL = StrSQL & "                      LongGranty , PayFirst, AmountAccept, Complaint, Noteinitial"
StrSQL = StrSQL & " From dbo.TblCarBillMentains"
    BolBegine = False
    StrWhere = ""
...
'////////////////////////
 If TxtPlateNo.Text <> "" Then
        ...
        End If
    End If
   If Me.DCEmp_Name.Text <> "" Then
        If BolBegine = True Then
            StrWhere = StrWhere & " AND ClientName like N'%" & Trim(Me.DCEmp_Name.Text) & "%'"
        Else
            BolBegine = True
            StrWhere = " Where  ClientName like N'%" & Trim(Me.DCEmp_Name.Text) & "%'"
        End If
    End If
...
Private Sub ChangeLang()
    ...
    Cmd(2).Caption = "Exit"
  Me.Caption = "Search BillCardAuthorizationReform"
...
' KeyAscii = KeyAscii_Num(TxtFrom, Me.TxtIDFrom.Text, 1)
End Sub
...
If IndTyp = 1 Then
With FrmBillCarMaintExtra.FG
.TextMatrix(Row, .ColIndex("valuedis")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("value"))
.TextMatrix(Row, .ColIndex("cod")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("id"))
If SystemOptions.UserInterface = ArabicInterface Then
.TextMatrix(Row, .ColIndex("name")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("name"))
Else
.TextMatrix(Row, .ColIndex("name")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("namee"))
End If
End With
ElseIf IndTyp = 2 Then
With FrmCarAuthontication.FG

.TextMatrix(Row, .ColIndex("nohours")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("Ling"))
.TextMatrix(Row, .ColIndex("value")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("value"))
.TextMatrix(Row, .ColIndex("cod")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("id"))
If SystemOptions.UserInterface = ArabicInterface Then
.TextMatrix(Row, .ColIndex("name")) = VSFlexGrid1.TextMatrix(VSFlexGrid1.Row, VSFlexGrid1.ColIndex("name"))
Else
