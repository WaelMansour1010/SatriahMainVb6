...
PercentgValueAddedAccount_Transec XPDtbTrans.value, 10, 1, account, Percetage
...
Private Sub ReLineGrid()
    ...
    With fg
        ...
        .TextMatrix(i, .ColIndex("totalm")) = val(.TextMatrix(i, .ColIndex("value"))) * val(.TextMatrix(i, .ColIndex("count")))
        Else
         .TextMatrix(i, .ColIndex("nett")) = .TextMatrix(i, .ColIndex("valuedis"))
        .TextMatrix(i, .ColIndex("totalm")) = .TextMatrix(i, .ColIndex("value"))
 ...
 If .TextMatrix(i, .ColIndex("value")) <> "" Then
                ...
                Me.lbTotalMente.Caption = val(Me.lbTotalMente.Caption) + val(fg.TextMatrix(i, fg.ColIndex("TotalNet")))
...
If AccountVat.BoundText = "" And mdifrmmain.taxes.Visible = True And CheckAnyVAT = True Then
...
Function print_report(Optional NoteSerial As String, Optional indexe As Integer)
    ...
    Dim Msg As String
MySQL = " SELECT     dbo.TblCarModels.Model, dbo.TblCarModels.CarID, dbo.TBLCarTypes.name, dbo.TBLCarTypes.namee, dbo.TblBranchesData.branch_name, "
...
MySQL = MySQL & "                       dbo.TblCarBillMentains.PaymentType, dbo.TblCustemers.CusName, dbo.TblCustemers.CusNamee, dbo.TblCustemers.CustGID, dbo.TblCarModels.ModelE,"
...
If KeyCode = vbKeyReturn Then
            ...
            If DcbBasedOn.ListIndex = 2 Then
                StrSQL = "select * From TblCardAuthorizationReform  WHERE AuthoOrder=" & val(txtAuthoOrder.Text) & "   Order By ID"
            End If
...
'''''''///////////////////
   ...
   ' txtCusId1.text = IIf(IsNull(rs("CusId").value), "", rs("CusId").value)
    ...
    Me.AccountVat.BoundText = IIf(IsNull(rs("AccountCodeVat").value), "", rs("AccountCodeVat").value)
        txtAuthoOrder.Text = IIf(IsNull(rs("AuthoOrder").value), "", val(rs("AuthoOrder").value))
...
fg.Clear flexClearScrollable, flexClearEverything
     ...
     End If
      If Rs1("OrderStatus").value = 3 Then
     ...
     End If
                         If Rs1("OrderStatus").value = 5 Then
     MsgBox " „ «·«‰ Â«¡ „‰ «·«„—    "
              clear_all Me
              fg.Clear flexClearScrollable, flexClearEverything
            fg.Rows = 2
 ...
 ' Me.TxtCusID.text = IIf(IsNull(Rs1("ClientCode").value), "", val(Rs1("ClientCode").value))
  ...
  'Me.TxtCusID.text = IIf(IsNull(Rs1("ClientCode").value), "", (Rs1("ClientCode").value))
    ...
    TxtReq.Text = IIf(IsNull(Rs1("WorkOrder").value), "", val(Rs1("WorkOrder").value))
    txtAuthoOrder.Text = IIf(IsNull(Rs1("AuthoOrder").value), "", val(Rs1("AuthoOrder").value))
...
Dim valuee As Double
 ...
 LngDevNO = 0
   ...
   If CboPayMentType.ListIndex = 0 Then '‰ﬁœÌ
        ...
        DebitAccount = ModAccounts.GetMyAccountCode("TblCustemers", "CusID", val(Me.DcbCustmer.BoundText), "Account_code")
        LngDevNO = LngDevNO + 1
        If ModAccounts.AddNewDev(LngDevID, LngDevNO, DebitAccount, valuee, 1, StrTempDes & "      Õ”«» «·‘—ﬂ… ", general_noteid, , , , NoteDate, UserID, , , , , , , , , , , , , , , , , , BranchID) = False Then
                GoTo ErrTrap
        End If
      End If
