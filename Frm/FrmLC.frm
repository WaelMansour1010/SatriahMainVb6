...
, NoteDate As Date)
    ...
    If Notevalue > 0 Then
        ...
        If ModAccounts.AddNewDev(LngDevID, line_no, StrAccountCodeDebt, Notevalue, 0, Msg & "    Õ”«» „’«—Ì› › Õ «·«⁄ „«œ  ", val(notes_id), , , , dbFromDate.value, val(DCboUserName.BoundText), , , , , , CLng(mRate), , , setfoxy_Line, , , , , , , , , _
        val(dcBranch.BoundText)) = False Then
            GoTo ErrTrap
        End If
        StrAccountCodeCridet = get_bank_Account(Dcbank.BoundText, "Account_Code")
        ...
        line_no = line_no + 1
        If ModAccounts.AddNewDev(LngDevID, line_no, StrAccountCodeCridet, val(TxtValue.Text) * mRate, 1, Msg & "    Õ”«»  «·»‰ﬂ ", val(notes_id), , , , dbFromDate.value, val(DCboUserName.BoundText), , , , , , CLng(mRate), , , setfoxy_Line, , , , , , , , , val(dcBranch.BoundText)) = False Then
            GoTo ErrTrap
        End If
        line_no = line_no + 1
    End If
...
'        End If
    ...
    If TxtModFlg.Text = "N" Then
        ...
        rs("Account_Code").value = ModAccounts.AddNewAccount(Account_Code_dynamic, "  «⁄ „«œ  " & "  " & TxtName.Text & "  »—ﬁ„ :" & Trim$(Me.TxtLCNO.Text), True, False, "  LC : " & TxtNameE.Text & "  NO :" & Trim$(Me.TxtLCNO.Text))
                    
    ElseIf Me.TxtModFlg.Text = "E" Then

        If Not IsNull(rs("Account_Code").value) Then
            ModAccounts.EditAccount rs("Account_Code").value, " «⁄ „«œ   " & TxtName.Text & "  »—ﬁ„ :" & Trim$(Me.TxtLCNO.Text), "LC:  " & TxtNameE.Text & "  No :" & Trim$(Me.TxtLCNO.Text), , , , , , , , , , , , , , , , , True
        End If
