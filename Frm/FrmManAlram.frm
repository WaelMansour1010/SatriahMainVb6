...
Private Sub ChangeLang()
    Me.Caption = "Alert Collection Devices"
    Ele.Caption = Me.Caption
    lbl(0).Caption = "No"
    lbl(1).Caption = "Bill#"
    lbl(2).Caption = "Date"
    lbl(3).Caption = "Periority"
    Fra.Caption = "Invoice Data"
    lbl(6).Caption = "Cust."
    lbl(10).Caption = "Value"
    lbl(4).Caption = "Remarks"
    lbl(5).Caption = "User"
    XPBtnOK.Caption = "Send"
    XPBtnCancel.Caption = "Cancel"

End Sub

Private Sub InvSerial_KeyDown(KeyCode As Integer, _
                              Shift As Integer)
...
Private Sub TxtInvID_Change()
    ...
    Else
        ...
        StrSQL = "SELECT  dbo.Transactions.Transaction_ID, dbo.Transactions.Transaction_Serial," & "dbo.Transactions.Transaction_Date, dbo.TblCustemers.CusName,dbo.QryOneTransactionTotal(" & "dbo.Transactions.Transaction_ID) AS TotalX "
        StrSQL = StrSQL + " FROM dbo.Transactions INNER JOIN dbo.TblCustemers ON " & "dbo.Transactions.CusID = dbo.TblCustemers.CusID "
...
Private Sub XPBtnOK_Click()
    ...
    StrSQL = "SELECT dbo.TblManAlram.TableID, dbo.TblManAlram.TransID, dbo.TblManAlram.AlramDate," & "dbo.TblManAlram.AlramPriority, dbo.TblManAlram.AlramMsg,dbo.TblManAlram.State," & "dbo.Transactions.Transaction_Serial, dbo.TblUsers.UserName "
    StrSQL = StrSQL + " FROM dbo.TblManAlram INNER JOIN dbo.Transactions ON dbo.TblManAlram.TransID" & "= dbo.Transactions.Transaction_ID INNER JOIN dbo.TblUsers ON dbo.TblManAlram.UserID =" & "dbo.TblUsers.UserID "
