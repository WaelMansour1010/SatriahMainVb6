...
For i = 1 To grdMaster2.Rows - 1
    ...
    If Not RsTrans.EOF Then
        ...
        Loop
        Cn.Execute "Delete Transactions where Transaction_ID = " & val(RsTrans!nots & "") & " and Transaction_Type=20"
...
'salimhere************************
 ...
 'Dim txtNoteSerial1V As String
        ...
        TxtNoteSerial1V = Voucher_coding(val(my_branch), mDate, 9, 160, , 20, , mStoreId, , , , mUserID)
        CurrentVoucherNo = ""
     ...
     sql22 = "INSERT INTO  Transactions (CBoBasedON,Transaction_ID ,Transaction_Serial,Transaction_Date,Transaction_Type ,CusID,StoreID,UserID,Emp_ID,nots,NoteSerial,NoteSerial1,NoteId,BranchId,nots2,TransactionComment)SELECT 5," & Transaction_ID & "," & MYTEXT & ",Transaction_Date,Transaction_Type = 20,CusID,StoreID,UserID,Emp_ID,nots=" & val(mOrderId) & ",NoteSerial='" & TxtNoteSerialV & "',NoteSerial1='" & TxtNoteSerial1V & "',NoteId=" & general_noteid & ",BranchId ," & TxtNoteSerial & "  ,TransactionComment  From Transactions Where Transaction_ID =" & mOrderId & " And Transaction_Type = 22"
     
     Cn.Execute sql22
    
    
       StrSQL = "UPDATE Transactions SET NOTS=" & Transaction_ID & " WHERE Transaction_ID=" & mOrderId
        Cn.Execute StrSQL
...
'   RsNotesGeneral.Open StrSQL, Cn, adOpenKeyset, adLockOptimistic, adCmdText
 ...
 '   RsNotesGeneral.update
    ...
    Dim sql As String
     ...
     sql = sql & " )+TotalDiscountPerLine)*QtyBySmalltUnit)*" & 1 & ")+(ToTAlELSHahn+LineShahn)*QtyBySmalltUnit) ,guaranteeTime," & Transaction_ID & ",Item_ID,ItemCase,ItemSerial , Quantity, (( ( round(Commisionvalue,2)+ Price-(round(discountvalue,2)+TotalDiscountPerLine))*" & val(1) & ")+(ToTAlELSHahn+LineShahn) ), ColorID,ItemSize, UnitId, ShowQty, QtyBySmalltUnit,BranchId,FoxyNo,OrderArrivalDate,order_no,ClassID," & mDate & ",ExpiryDate,LotNO ,OldQty,OldCost,NewQty,NewCost ,length,OUTR,INR,Height,Width ,NoCount From dbo.Transaction_Details Where Transaction_ID =" & mOrderId
     

    Cn.Execute sql
   ' SaveItemsData (Transaction_ID,,mOrderId) ' ›«’Ì· «·«’‰«›
...
Dim SngTemp As Double
    SngTemp = mTotalCost
     CreateNotes NoteID, mDate, my_branch, 160, SngTemp, NoteSerial, TxtNoteSerial1V, "Transactions", "Transaction_ID", Transaction_ID, TxtNoteSerial1V, ToHijriDate(mDate)
          ' TxtNoteID.text = NoteID
     general_noteid = NoteID
     CREATE_VOUCHER_GE Transaction_ID, TxtNoteSerialV, TxtNoteSerial1V, general_noteid, val(my_branch)
    End If
    IsVouc = True
...
Function SaveItemsData(Optional Transaction_ID As String = 0, Optional StoreID3 As Integer, Optional mOrderId As Integer = 0)
If SystemOptions.WorkWithItemsDetails = False Then Exit Function
    ...
    Cn.Execute "delete ItemsDetails   where Transaction_ID= " & val(mOrderId)
