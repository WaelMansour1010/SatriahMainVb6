...
MonthDayNo = daysInMonth(date2.value)
        ...
        .TextMatrix(.rows - 1, .ColIndex("total1")) = SngTotal
         TxtSalary = SngTotal
               ...
               If ChSalar.value = xtpChecked Then
    txtCount.text = 0
    TxtSalary = 0
     End If
...
End If
 If ChSalar.value = xtpUnchecked Then
txtSal.text = 0
 End If
        ...
        For j = 1 To 40
            ...
            If ViewComp(j) = True And AddOrDiscount(j) = 0 Then '«·ŸÂÊ— Ê«÷«›… Ê·Ì” –„„ Ê·Ì” „ﬁœ„
                ...
                If ZmamAccount(j) <> True And AdvPaymentdAccount(j) <> True Then
                        ...
                        If CValue > 0 And ChSalar.value = xtpChecked Then
...
'                    Next branch
    ...
    Benfts = Round(Benfts, 2)
            If Benfts > 0 And val(val(txtCount.text)) <> 0 And ChSalar.value = xtpChecked Then        '«·«ÃÊ— «·„” Õﬁ… œ«∆‰
...
sql = sql & " WHERE   dbo.TblEmployee.Emp_ID = " & val(DcboEmp.BoundText) & " and   (dbo.mofrad.ZmamAccount = 0) AND (MONTH(dbo.TblChangedComponentRegister.RecordDate) = MONTH(" & SQLDate(NoteDate, True) & " )) AND"
...
sql = sql & " dbo.mofrad ON dbo.TblChangedComponentRegister.ComponentID = dbo.mofrad.id LEFT OUTER JOIN"
sql = sql & " dbo.projects ON dbo.TblChangedComponentRegisterDetails.projectid = dbo.projects.id"
sql = sql & " WHERE    dbo.TblEmployee.Emp_ID = " & val(DcboEmp.BoundText) & " and    (dbo.mofrad.ZmamAccount = 1) AND (MONTH(dbo.TblChangedComponentRegister.RecordDate) = MONTH(  " & SQLDate(NoteDate, True) & " )) AND"
sql = sql & " (YEAR(dbo.TblChangedComponentRegister.RecordDate) = YEAR( " & SQLDate(NoteDate, True) & " ))"
...
sql = sql & " GROUP BY dbo.TblEmployee.Emp_ID,dbo.mofrad.Account_Code, dbo.EmpSalaryComponent.EntIncresDataM, dbo.projects.Salary_account, 2006 + dbo.opr_Employee.Years, dbo.opr_Employee.Months,"
...
sql = sql & "  dbo.EmpSalaryComponent.EntIncresDataM >= " & SQLDate(NoteDate, True) & " )"
  sql = sql & "  and dbo.TblEmployee.Emp_ID = " & val(DcboEmp.BoundText) & " "
sql = sql & "   AND (dbo.opr_Employee.Months = " & CmbMonth(0).ListIndex & ") AND (2006 + dbo.opr_Employee.Years = " & val(CboYear(0).text) & ")"
...
sql = sql & " dbo.mofrad ON dbo.TblChangedComponentRegister.ComponentID = dbo.mofrad.id LEFT OUTER JOIN"
sql = sql & " dbo.projects ON dbo.TblChangedComponentRegisterDetails.projectid = dbo.projects.id"
sql = sql & " WHERE    dbo.TblEmployee.Emp_ID = " & val(DcboEmp.BoundText) & " and (dbo.mofrad.AdvPaymentdAccount = 1) AND (MONTH(dbo.TblChangedComponentRegister.RecordDate) = MONTH(   " & SQLDate(NoteDate, True) & "  )) AND"
sql = sql & " (YEAR(dbo.TblChangedComponentRegister.RecordDate) = YEAR( " & SQLDate(NoteDate, True) & "  ))"
sql = sql & " GROUP BY dbo.TblChangedComponentRegisterDetails.projectid, dbo.projects.Salary_account, dbo.projects.Project_name, dbo.mofrad.name,"
sql = sql & " dbo.TblChangedComponentRegister.BranchId, dbo.mofrad.AddOrDiscount, dbo.TblEmployee.Emp_Code, dbo.TblEmployee.Emp_Name,"
...
Me.C1Tab1.Caption = "Data|Approve Status"
