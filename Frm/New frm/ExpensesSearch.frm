...
Private Sub fg_Click(index As Integer)
    ...
    If Not FG(Indx).TextMatrix(FG(Indx).row, 1) = "" Then
        ...
        ElseIf Me.RetrunType = 22 Then
           ...
           ElseIf Me.RetrunType = 2508 Then
            FrmPayments1.DBCboClientName.BoundText = (FG(Indx).TextMatrix(FG(Indx).row, 5))
        ElseIf Me.RetrunType = 2509 Then
            FrmCashing.DcboBox.BoundText = (FG(Indx).TextMatrix(FG(Indx).row, 1))
        ElseIf Me.RetrunType = 2510 Then
            FrmCashing.DcboBankName.BoundText = (FG(Indx).TextMatrix(FG(Indx).row, 1))
        ElseIf Me.RetrunType = 2511 Then
            FrmPayments.DcboBox.BoundText = (FG(Indx).TextMatrix(FG(Indx).row, 1))
        ElseIf Me.RetrunType = 2512 Then
            FrmPayments.DcboBankName.BoundText = (FG(Indx).TextMatrix(FG(Indx).row, 1))
...
StrSQL = StrSQL + "  Where (TT.BoxID <> 0)"
    ...
    End If
     If Me.dcBranch(1).text <> "" Then
        StrWhere = StrWhere + " and tt.BranchId =" & val(Me.dcBranch(1).BoundText)
    End If
    
    If Me.DcEmp.text <> "" Then
        StrWhere = StrWhere + " and EmpID = " & val(Me.DcEmp.BoundText)
    End If

If (Me.tXTAccount_Serial(2).text) <> "" Then
        StrSQL = StrSQL + " AND ACCOUNTS.account_serial  like '%" & (Me.tXTAccount_Serial(2).text) & "%'"
    End If
