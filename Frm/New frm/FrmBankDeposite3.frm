         ...
         AccountCode = salesAccount
                vaTAccount = VatsalesAccount
                  ...
                  '  val (mValue - val(.TextMatrix(i, .ColIndex("chkTaxExempt"))))
                    If ModAccounts.AddNewDev(LngDevID, line_no, AccountCode, val(mValue2), 1, .TextMatrix(i, .ColIndex("Remarks")) & "ﬁÌ„… „÷«›…", val(NoteID), , , SystemOptions.SysCurrentAccountIntervalID, Me.dbRecordDate.value, user_id, , , , .TextMatrix(i, .ColIndex("NetValue")), , , , bankDes, , , , , , , , , , val(Me.Dcbranch.BoundText)) = False Then
                        GoTo ErrTrap
...
Private Sub cmdDisplayFix_Click()
    ...
    '-----------------------------------------------------------------
    ' 2) ≈œŒ«· ≈Ã„«·Ì «·„œ›Ê⁄ „‰ TblTransactionPayments ·ﬂ· ›« Ê—…
    '    (ﬂ· «·ÿ—ﬁ: ﬂ«‘° ›Ì“«° ... ≈·Œ)
    '-----------------------------------------------------------------
    s = ""
    ...
    '-----------------------------------------------------------------
    ' 3) ≈œŒ«· «·›Ê« Ì— ﬂ«‘ «·„»«‘—… (PaymentType = 0)
    '    »‘—ÿ √·« ÌﬂÊ‰ ·Â« √Ì ”ÿÊ— ›Ì TblTransactionPayments
    '    (⁄‘«‰ „«   ﬂ——‘ „⁄ «··Ì ›Êﬁ)
    '-----------------------------------------------------------------
    s = ""
    s = s & " INSERT INTO tmpPos33 (Transaction_ID, TotalNetValue)"
    ...
    '-----------------------------------------------------------------
    ' 4) NetValue0 = ﬂ«‘ (PaymentID = 0) + ›Ê« Ì— ﬂ«‘ »œÊ‰ ”ÿÊ— œ›⁄
    '-----------------------------------------------------------------
    s = ""
    s = s & " UPDATE tmpPos33"
    ...
    '-----------------------------------------------------------------
    ' 7) —»ÿ √Ê· ID ·ﬂ· ‰Ê⁄ œ›⁄ (ID0 ··ﬂ«‘ + ID1/ID2/ID3 Õ”» PaymentType)
    '-----------------------------------------------------------------
    ' ID0 ? PaymentID = 0 (ﬂ«‘  ›’Ì·Ì „‰ TblTransactionPayments)
    s = ""
    s = s & " UPDATE tmpPos33"
    s = s & " SET ID0 = ("
    ...
    '-----------------------------------------------------------------
    ' 8) BoxID / CurrentCashireID / PointID „‰ √Ê· ”ÿ— œ›⁄
    '    (‰›” ·ÊÃÌﬂﬂ «·√’·Ì ñ ·„ √·„”Â ⁄‘«‰ „« ‰ﬂ”—‘ Õ«Ã…)
    '-----------------------------------------------------------------
    s = ""
    s = s & " UPDATE tmpPos33"
    ...
    ' „·«ÕŸ…: Â‰« √‰  ﬂ‰  » „·√ PointID »‰›” CurrentCashireID
    ' ·Ê œÂ „ﬁ’Êœ ”Ì»Â° ·Ê ·√ „„ﬂ‰  ⁄œ·Â« ·«Õﬁ« ·‹ tp.PointID
    s = ""
    s = s & " UPDATE tmpPos33"
    s = s & " SET PointID = ("
    s = s & "     SELECT TOP 1 tp.CurrentCashireID"
...
For i = 1 To grdPay.Cols - 1
    ...
    If grdPay.ColHidden(i) = False Then
        Dim isEmptyColumn As Boolean
        isEmptyColumn = True ' ‰»œ√ »«› —«÷ √‰ «·⁄„Êœ ›«—€

        For j = 1 To grdPay.rows - 1
            If Trim(grdPay.TextMatrix(j, i)) <> "" Then
                isEmptyColumn = False ' «·⁄„Êœ ÌÕ ÊÌ ⁄·Ï ﬁÌ„… €Ì— ›«—€…
                Exit For ' ·« Õ«Ã… ·„Ê«’·… «· Õﬁﬁ
            End If
        Next j

        grdPay.ColHidden(i) = isEmptyColumn ' ≈Œ›«¡ «·⁄„Êœ ≈–« ﬂ«‰ ›«—€« »«·ﬂ«„·
    End If
Next i
...
Private Sub SaveFix( _
    ...
    ' ·Ê «·ﬁÌ„… „‘ ’›— ? «„”Õ ﬂ· ”ÿÊ— Â–« «·‹PaymentID ··›« Ê—… Ê√⁄œ ≈œŒ«· ”ÿ— Ê«Õœ ÃœÌœ
    If netvalue <> 0 Then
...
ContinueItem:
        ...
        ' 2) »⁄œ „« ‰ﬂ„¯· ﬂ· ›Ê« Ì— «·”‰œ: ‘€¯· createVoucher ··”‰œ ‰›”Â („—… Ê«Õœ…)
...
Dim s As String
        ...
        createVoucher           ' ·Ê » Õ «Ã »«—«„ —« ° ”Ì»Â« “Ì „« » ” Œœ„Â« ›Ì «·‘«‘… «·Õ«·Ì…
        Cn.CommitTrans
    ...
    MsgBox " „  «·„⁄«·Ã…." & vbCrLf & _
           "”‰œ«  «·’—› (‰Ã«Õ/›‘·): " & doneIssue & " / " & failIssue & vbCrLf & _
           "createVoucher (‰Ã«Õ/›‘·): " & doneVoucher & " / " & failVoucher, vbInformation
    Exit Sub
...
Public Sub XPBtnMoveNextAuto()
    ...
    rs.MoveNext
    If rs.EOF Then Exit Sub   ' ? „›Ì‘ MoveLast ›Ì «·√Ê Ê„« Ìﬂ
    Retrive
End Sub
...
Public Function ReCreateIssueVoucherFromSale(ByVal SaleTransactionID As Long, _
    ...
    If RSH.EOF Then
        MsgBox "·„ Ì „ «·⁄ÀÊ— ⁄·Ï ÂÌœ— «·›« Ê—…: " & SaleTransactionID, vbExclamation
        Exit Function
    End If
   ...
   ' Cn.BeginTrans
    ...
    '========================================================
    ' 2)  Ê·Ìœ √—ﬁ«„ ÃœÌœ… + √ﬂÊ«œ «·”‰œ/«·‰Ê  »‰«¡ ⁄·Ï (BranchID,BillDate)
    '========================================================
    ...
    If vNoteSerial1 = "error" Or Len(Trim$(vNoteSerial1)) = 0 Then
        Err.Raise vbObjectError + 7771, , "·« Ì„ﬂ‰ ≈‰‘«¡ —ﬁ„ ”‰œ «·’—› (NoteSerial1)."
    End If

    '========================================================
    ' 3) ≈‰‘«¡ ÂÌœ— Õ—ﬂ… «·’—› (19) „‰ ÂÌœ— «·›« Ê—… (21) „»«‘—…
    '========================================================
    s = ""
    ...
    '========================================================
    ' 4)  ›«’Ì· «·’—›: „‰  ›«’Ì· «·›« Ê—… (21) „⁄ Õ”«» «· ﬂ·›… „‰ SQL
    '    Price = dbo.fn_GetItemCost(..., @BillDate „‰ ÂÌœ— «·›« Ê—…, @StoreID „‰ «·ÂÌœ—)
    '========================================================
    s = ""
    ...
    ' ≈‰‘«¡ «·‰Ê  (NoteType=180 Õ”» ﬂÊœﬂ) Ê ÕœÌÀ «·ÂÌœ— »«·ﬁÌ„…
...
With Cmd
    ...
    pDesc.value = Null ' Œ·ÌÂ Null ⁄‘«‰ «·Ê’› «·«› —«÷Ì
