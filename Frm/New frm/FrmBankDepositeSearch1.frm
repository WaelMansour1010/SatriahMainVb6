...
Begin VB.Form FrmBankDepositeSearch1 
   Appearance      =   0  'Flat
   BackColor       =   &H00E2E9E9&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "«·»ÕÀ ⁄‰  ‘Ìﬂ«   Õ  «· Õ’Ì· «Ê ‘Ìﬂ«  „ƒÃ·…"
   ClientHeight    =   5175
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   13065
   Icon            =   "FrmBankDepositeSearch1.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   RightToLeft     =   -1  'True
   ...
   Begin VSFlex8UCtl.VSFlexGrid Fg 
      ...
      FormatString    =   $"FrmBankDepositeSearch1.frx":038A
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin ImpulseButton.ISButton CmdShowMoreOptions 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      ButtonImage     =   "FrmBankDepositeSearch1.frx":063F
      ColorHoverText  =   12582912
      ButtonToggles   =   1
      DrawFocusRectangle=   0   'False
      RightToLeft     =   -1  'True
      ButtonImageToggled=   "FrmBankDepositeSearch1.frx":09D9
   ...
   End
End
Attribute VB_Name = "FrmBankDepositeSearch1"
            ...
            Else
                ...
                With Me.FG
                    ...
                    For i = .FixedRows To rs.RecordCount
                        ...
                        .TextMatrix(i, .ColIndex("NoteID")) = IIf(IsNull(rs("id").value), "", rs("id").value)
                        ...
                        If Not IsNull(rs("BankDate").value) Then
                            .TextMatrix(i, .ColIndex("TranDate")) = DisplayDate(rs("BankDate").value)
                        End If
                        ...
                        If Not IsNull(rs("OperationType").value) Then
                            If rs("OperationType").value = 0 Then
                                ...
                                .TextMatrix(i, .ColIndex("PaymentType")) = "   Õ’Ì· ‘Ìﬂ ··‘—ﬂ… "
                            ElseIf rs("OperationType").value = 1 Then
                                .TextMatrix(i, .ColIndex("PaymentType")) = " ”œ«œ ‘Ìﬂ ⁄·Ï «·‘—ﬂ…  "
                            ElseIf rs("OperationType").value = 2 Then
                                .TextMatrix(i, .ColIndex("PaymentType")) = " ‘Ìﬂ „— œ ⁄·Ï ⁄„Ì· «Ê „Ê—œ "
                            ElseIf rs("OperationType").value = 3 Then
                                .TextMatrix(i, .ColIndex("PaymentType")) = " ‘Ìﬂ „— œ ⁄·Ï «·‘—ﬂ…  "
                                
                            End If
                        End If
                   
                        .TextMatrix(i, .ColIndex("DepositeBank")) = IIf(IsNull(rs("BankName").value), "", rs("BankName").value)
...
Private Sub Fg_Click()
    ...
    With Me.FG
        ...
        FrmBankDeposite1.Retrive val(.TextMatrix(.Row, .ColIndex("NoteID")))
...
Private Function Build_Sql() As String
    ...
    If SystemOptions.SysDataBaseType = SQLServerDataBase Then
        ...
        StrSQL = "SELECT  dbo.TblBanksCollect.id,   dbo.TblBanksCollect.RecordDate, dbo.TblBanksCollect.bankid, dbo.TblBanksCollect.NoteSerial1, dbo.TblBanksCollect.NoteSerial, dbo.TblBanksCollect.NoteID, "
        StrSQL = StrSQL & "  dbo.TblBanksCollect.BankDate, dbo.TblBanksCollect.OperationType, dbo.TblBanksCollectDetails.[value], dbo.TblBanksCollectDetails.ChequeNo,"
        StrSQL = StrSQL & "  dbo.TblBanksCollectDetails.REMARKS , dbo.TblBanksCollectDetails.DueDate, dbo.BanksData.BankName"
        StrSQL = StrSQL & " FROM         dbo.TblBanksCollect INNER JOIN"
        StrSQL = StrSQL & " dbo.TblBanksCollectDetails ON dbo.TblBanksCollect.id = dbo.TblBanksCollectDetails.TblBanksCollectId LEFT OUTER JOIN"
        StrSQL = StrSQL & " dbo.BanksData ON dbo.TblBanksCollect.bankid = dbo.BanksData.BankID"
    ...
    If DCboCashType.ListIndex >= 0 And DCboCashType.ListIndex < 4 Then ' ‰Ê⁄ «·⁄„·Ì…
 
        StrWhere = StrWhere + " AND  (OperationType=" & val(DCboCashType.ListIndex) & ")"
    ...
    If Me.DcboBankName.BoundText <> "" Then
        ...
        StrWhere = StrWhere + " AND TblBanksCollect.bankid =" & val(Me.DcboBankName.BoundText) & ""
    ...
    If Not IsNull(Me.DTPTranDate.value) Then '   «—ÌŒ «·Õ—ﬂ…
        StrWhere = StrWhere + " AND  BankDate =" & SQLDate(Me.DTPTranDate.value, True) & ""
    End If
 
    If Trim(Me.TxtRemarks.text) <> "" Then
        
        StrWhere = StrWhere + " AND TblBanksCollectDetails.Remarks like'%" & (Me.TxtRemarks.text) & "%'"
