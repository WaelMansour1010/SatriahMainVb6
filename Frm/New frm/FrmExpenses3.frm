...
Function print_report(Optional NoteSerial As String, Optional VendorName As String)
    ...
    If CboPaymentType1.ListIndex = 0 Then
        MySQL = "SELECT   distinct  DOUBLE_ENTREY_VOUCHERS.DEV_ID_Line_No , dbo.notes_all.NoteID, DOUBLE_ENTREY_VOUCHERS.VAT ,dbo.notes_all.NoteDate, dbo.notes_all.NoteType, dbo.notes_all.NoteSerial, dbo.notes_all.Note_Value, dbo.notes_all.BankID, "
        ...
        MySQL = MySQL & "  dbo.TblUsers.UserName, dbo.TblBoxesData.BoxName, dbo.BanksData.BankName, dbo.notes_all.too, dbo.DOUBLE_ENTREY_VOUCHERS.Value AS [Sub-value],"
    ...
    Else
        ...
        MySQL = " SELECT  distinct DOUBLE_ENTREY_VOUCHERS.DEV_ID_Line_No, Notes.NoteDate, Notes.NoteType, Notes.NoteSerial, Notes.NoteSerial1, Notes.BankID, Notes.ChqueNum, Notes.DueDate, Notes.CusID, Notes.BoxID, Notes.Note_Value, Notes.note_value_by_characters,"
        ...
        MySQL = MySQL & " , dbo.DOUBLE_ENTREY_VOUCHERS.Value AS [Sub-value], dbo.notes.ExpensesRemark"
    ...
    End If
MySQL = MySQL & " order by DOUBLE_ENTREY_VOUCHERS.DEV_ID_Line_No "
...
Private Sub CmdRemove_Click()
    Dim X As Integer
 If CboPaymentType1.ListIndex = 0 Then
If val(Fg_Journal.TextMatrix(Fg_Journal.row, Fg_Journal.ColIndex("FlgVat"))) = 1 And SystemOptions.AllowEditVaTManulay = False Then
If SystemOptions.UserInterface = ArabicInterface Then
...
If val(VSFlexGrid1.TextMatrix(VSFlexGrid1.row, VSFlexGrid1.ColIndex("FlgVat"))) = 1 And SystemOptions.AllowEditVaTManulay = False Then
If SystemOptions.UserInterface = ArabicInterface Then
MsgBox "·«Ì„ﬂ‰ Õ–› ”ÿ— «·›«  .Ì—ÃÏ  ’›Ì— ‰”»… «·›« "
Else
MsgBox "Can not delete VAT  "
...
StrSQL = StrSQL + "              TblEmpDepartments On TblEmpDepartments.DeparmentID = DOUBLE_ENTREY_VOUCHERS.Departementid "
      StrSQL = StrSQL + " Where (dbo.DOUBLE_ENTREY_VOUCHERS.hideline = 0 or dbo.DOUBLE_ENTREY_VOUCHERS.hideline is null)and (dbo.DOUBLE_ENTREY_VOUCHERS.Credit_Or_Debit = 0) And (dbo.DOUBLE_ENTREY_VOUCHERS.notes_id = " & val(rs("A_NoteID").value) & ")"
      StrSQL = StrSQL + "  and IsNull(DOUBLE_ENTREY_VOUCHERS.IsHidden,0) = 0 "
        StrSQL = StrSQL + " ORDER BY dbo.DOUBLE_ENTREY_VOUCHERS.DEV_ID_Line_No"
   ...
   RsDev.Open StrSQL, Cn, adOpenKeyset, adLockOptimistic, adCmdText
          ...
          '  RsDev.Open "DOUBLE_ENTREY_VOUCHERS", Cn, adOpenStatic, adLockOptimistic, adCmdTable
            ...
            With Fg_Journal
                ...
                For i = .FixedRows To .rows - 1
                    'project_id = get_project_id(dcproject.BoundText, "expanses_account")
                If project_id = 0 Or val(DCproject.BoundText) = 0 Then
                    project_id = val(.TextMatrix(i, .ColIndex("projectid")))
                End If
...
'                            Material_account = get_project_Account(project_id, "AccountUnderImp")
                            If Material_account = "" Then
                                ...
                                StrSQL = "Select A14 from branches "
                                rsDummyAcc.Open StrSQL, Cn, adOpenStatic, adLockReadOnly
                        ...
                        If ModAccounts.AddNewDev(LngDevID, line_no, Material_account, val(.TextMatrix(i, .ColIndex("value"))), 0, .TextMatrix(i, .ColIndex("des")), val(NoteID), , , SystemOptions.SysCurrentAccountIntervalID, Me.XPDtbTrans.value, Me.DCboUserName.BoundText, , , , val(.TextMatrix(i, .ColIndex("value"))), Dccurrency.text, val(txt_Currency_rate.text), , , val(.TextMatrix(i, Fg_Journal.ColIndex("LineNo1"))), val(Me.XPTxtID.text), project_id, .TextMatrix(i, Fg_Journal.ColIndex("pandid")), , , , , , val(Me.dcBranch.BoundText), val(.TextMatrix(i, .ColIndex("CarId"))), , , , , , val(.TextMatrix(i, .ColIndex("DeparmentID"))), , , , , , .TextMatrix(i, .ColIndex("BillNo")), , val(.TextMatrix(i, .ColIndex("pandid"))), val(.TextMatrix(i, .ColIndex("operid"))), , , , , , , , , Posted, , OtherInformation, , True) = False Then
