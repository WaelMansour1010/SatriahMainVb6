...
sql = sql & " Where (dbo.Notes.NoteSerial1 = " & val(TxtVouSerial.text) & ") And (dbo.Notes.NoteType = 3) And (dbo.DOUBLE_ENTREY_VOUCHERS.Credit_Or_Debit = 0) and IsNull(FlgVat,0) <> 1"
If sql <> "" Then
Rs9.Open sql, Cn, adOpenStatic, adLockOptimistic, adCmdText
If Rs9.RecordCount > 0 Then
Rs9.MoveFirst
...
UpdateSql = " update  TblAdditionsAssest set NoteID =" & val(notes_id)
UpdateSql = UpdateSql & "  , NoteSerial ='" & TxtNoteSerial.text & "'"

UpdateSql = UpdateSql & " where id=" & val(XPTxtID.text)
...
Set Rs5 = New ADODB.Recordset
If NoteType <> 80 Then
    sql = " SELECT     NoteType,DOUBLE_ENTREY_VOUCHERS.Value as  Note_Value, NoteSerial1, NoteID"
    sql = sql & " From dbo.notes_all"
    sql = sql & " INNER JOIN DOUBLE_ENTREY_VOUCHERS ON DOUBLE_ENTREY_VOUCHERS.Notes_ID = notes_all.A_NoteID"
    sql = sql & " Where (NoteType = " & NoteType & ") And (NoteSerial1 = " & NoteSerial1 & ")AND (AssestPayd IS NULL) "
Else

    
    sql = " SELECT"
    sql = sql & "     sum(DOUBLE_ENTREY_VOUCHERS.value)   As Note_Value"
    sql = sql & "    ,notes_all.NoteSerial1"
    sql = sql & "    ,notes_all.NoteID"
       
    sql = sql & " From notes_all"
    sql = sql & " INNER JOIN Notes"
    sql = sql & "     ON notes_all.NoteID = Notes.notes_all"
    sql = sql & " INNER JOIN DOUBLE_ENTREY_VOUCHERS"
    sql = sql & "     ON Notes.NoteID = DOUBLE_ENTREY_VOUCHERS.Notes_ID"
    
    sql = sql & " Where (notes_all.NoteType = " & NoteType & ") And (notes_all.NoteSerial1 = " & NoteSerial1 & ")AND (notes_all.AssestPayd IS NULL) "
    
    sql = sql & " AND ISNULL(DOUBLE_ENTREY_VOUCHERS.FlgVat, 0) = 0"
    sql = sql & " AND Credit_Or_Debit = 0"
    sql = sql & " GROUP by notes_all.NoteSerial1"
    sql = sql & "    ,notes_all.NoteID"
End If
Rs5.Open sql, Cn, adOpenStatic, adLockOptimistic, adCmdText
If Rs5.RecordCount > 0 Then
