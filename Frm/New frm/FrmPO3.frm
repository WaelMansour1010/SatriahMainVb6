   ...
   Begin C1SizerLibCtl.C1Elastic EleMain 
      ...
      Begin C1SizerLibCtl.C1Elastic Ele 
         ...
         Begin VB.ComboBox CBoBasedON 
            ...
            Width           =   1770
         End
         Begin VB.CheckBox chkIsFirstInv 
            Alignment       =   1  'Right Justify
            Caption         =   "Check2"
            Height          =   255
            Left            =   240
         ...
         Begin VB.TextBox TxtCashCustomerName 
            ...
            Width           =   2535
         End
         Begin VB.TextBox TxtCusBalance 
            Alignment       =   1  'Right Justify
            BackColor       =   &H00C0FFFF&
            Height          =   285
            Left            =   600
         ...
         Begin ImpulseButton.ISButton CmdConvert 
            ...
            ColorTextShadow =   -2147483637
         End
         Begin MSDataListLib.DataCombo Dccurrency33 
            Height          =   315
            Left            =   3840
            TabIndex        =   78
            Top             =   120
...
Dim cSearchDcbo(3)   As clsDCboSearch
 ...
 Public IschkIsFirstInv As Boolean
...
LblFinal3 = Round(val(LblFinal2.Caption) / val(txt_Currency_rate), 2)
End If
...
Private Sub DcCurrency_Click(Area As Integer)
    DcCurrency_Change
End Sub
 Function GetOmrDaeen() As String
 ...
 Dim str2 As String
 str2 = ""
 sql = " SELECT      XB.Transaction_Type, XB.ID, SUM(XB.TransNet) AS SumTransNet, dbo.Ageng_type.Name, dbo.Ageng_type.[From], dbo.Ageng_type.[To], "
 sql = sql & "                      dbo.TransactionTypes.TransactionTypeName"
 sql = sql & "   FROM         (SELECT     Transaction_Type, dbo.GetDeptAgeID(DATEDIFF(day, DueDate, " & SQLDate(XPDtbBill.value, True) & ")) AS ID, ISNULL(Transaction_NetValue, 0) + ISNULL(OldValue, 0)"
 sql = sql & "                                             - ISNULL(dbo.GetPayValue(Transaction_ID), 0) AS TransNet, Datediff(day, DueDate, " & SQLDate(XPDtbBill.value, True) & ") AS DiffDate"
 sql = sql & "                       From dbo.transactions"
 sql = sql & "                       WHERE     (PaymentType = 1)and CusID =" & val(DBCboClientName.BoundText) & " and(PaymentType = 1) AND (Transaction_Type = 21 OR"
 sql = sql & "                                             Transaction_Type = 2 OR"
 sql = sql & "                                             Transaction_Type = 71) AND (TotalPayed IS NULL OR"
 sql = sql & "                                             TotalPayed = 0) AND (DATEDIFF(day, " & SQLDate(XPDtbBill.value, True) & ", DueDate) < 0)) XB LEFT OUTER JOIN"
 sql = sql & "                     dbo.TransactionTypes ON XB.Transaction_Type = dbo.TransactionTypes.Transaction_Type LEFT OUTER JOIN"
 sql = sql & "                     dbo.Ageng_type ON XB.ID = dbo.Ageng_type.id"
 sql = sql & "  GROUP BY XB.Transaction_Type, XB.ID, dbo.Ageng_type.Name, dbo.Ageng_type.[From], dbo.Ageng_type.[To], dbo.TransactionTypes.TransactionTypeName"
 sql = sql & "   Having (SUM(Xb.TransNet) <> 0)"
 sql = sql & "     ORDER BY XB.ID"
 rs2.Open sql, Cn, adOpenStatic, adLockOptimistic, adCmdText
 If rs2.RecordCount > 0 Then
 For i = 1 To rs2.RecordCount
 str2 = str2 & "«·ﬁÌ„…"
 str2 = str2 & " = "
 str2 = str2 & IIf(IsNull(rs2("SumTransNet").value), 0, rs2("SumTransNet").value)
 str2 = str2 & "Õ«·… ⁄„— «·œÌ‰"
 str2 = str2 & " / "
 str2 = str2 & IIf(IsNull(rs2("Name").value), 0, rs2("Name").value)
 str2 = str2 & CHR(13)
 Next i
 End If
 GetOmrDaeen = str2
...
Private Sub Cmd_Click(Index As Integer)
    ...
    If Voucher_coding(val(dcBranch.BoundText), XPDtbBill.value, 30, 0, , 6, , , , , , val(DCboUserName.BoundText), 0) = "" Then
...
Accredit.Enabled = True
        ...
        Case 5
         ...
         Order_no_search.lblSpecificsearch = 6
            ...
            'Order_no_search.DealingForm = GridTransType.ShowPrice
            'Order_no_search.Caption = "«·»ÕÀ ⁄‰   «Ê«„— «·»Ì⁄"
            'Order_no_search.Show vbModal
...
Private Sub DBCboClientName_Change()
    ...
    If Me.TxtModFlg.text = "N" Or Me.TxtModFlg.text = "E" Then
        ...
        WriteCustomerBalPublic AdvancedAccount, , balanceString
        TxtCusBalance.text = balanceString
...
Private Sub DBCboClientName_KeyUp(KeyCode As Integer, _
    ...
    If KeyCode = vbKeyF3 Then
        FrmCustemerSearch.SearchType = 14
...
Private Sub DCboStoreName_Change()
 TxtStoreID.text = getStoreCoding(val(DCboStoreName.BoundText))
    ...
    If Me.TxtModFlg = "N" Or Me.TxtModFlg = "E" Then
     ...
     If CheckStoreCoding(val(dcBranch.BoundText), 30) = True Then
...
ShowAttachments TxtNoteSerial1, "060520154"
...
Public Sub RetriveOrder(Optional order_no As String = "", _
                        Optional Transaction_Type As Integer = 0, Optional str As String)
    ...
    FG.Refresh
        ...
        ElseIf CBoBasedON.ListIndex = 3 Then
            StrSQL = "Select * from transactions where  Transaction_Type=29 and NoteSerial1='" & order_no & "'"
        Else
            StrSQL = "Select * from transactions where  Transaction_Type=42 and Order_no='" & order_no & "'"
        End If
    ...
    Else
       ...
       TxtTransactionComment.text = IIf(IsNull(rs("TransactionComment").value), "", (rs("TransactionComment").value))
...
poTransaction_ID.text = IIf(IsNull(rs("poTransaction_ID").value), "", (rs("poTransaction_ID").value))
        ...
        If Trim(rs!TransactionComment & "") <> "" Then
            TxtTransactionComment.text = IIf(IsNull(rs("TransactionComment").value), "", (rs("TransactionComment").value))
        Else
            TxtTransactionComment.text = IIf(IsNull(rs("Remark").value), "", (rs("Remark").value))
        End If
Transaction_ID = val(rs("Transaction_ID").value & "")
...
If CBoBasedON.ListIndex = 3 Then
    StrSQL = "SELECT TblItems.HaveSerial, dbo.[GetBalanceQtyPO4] (Transaction_Details.Item_ID,N'" & Trim(order_no) & "'," & val(Me.XPTxtBillID) & ",6,3) as Showqty6 , * FROM TblItems INNER JOIN Transaction_Details " & "ON TblItems.ItemID = Transaction_Details.Item_ID INNER JOIN dbo.TblUnites ON dbo.Transaction_Details.UnitID = dbo.TblUnites.UnitID"
End If
...
Exit Sub
 End If
 If CBoBasedON.ListIndex = 3 Then
    StrSQL = StrSQL + " And dbo.[GetBalanceQtyPO4] (Transaction_Details.Item_ID,N'" & Trim(order_no) & "'," & val(Me.XPTxtBillID) & ",6,3) <> 0"
Else
...
End If
    ...
    If Not (RsDetails.EOF Or RsDetails.BOF) Then
        ...
        For Num = 1 To RsDetails.RecordCount
            ...
            FG.TextMatrix(Num, FG.ColIndex("Count")) = val(RsDetails!ShowQty6 & "")
            FG.TextMatrix(Num, FG.ColIndex("StillQty")) = val(RsDetails!ShowQty6 & "")
               FG.TextMatrix(Num, FG.ColIndex("OldID")) = IIf(IsNull(RsDetails("LineId")), "", (RsDetails("LineId").value))
...
End Sub
Private Sub TxtAdvPay_KeyUp(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyF3 Then
    
    FrmSearchDevComItem.CusID = val(DBCboClientName.BoundText)
          Load FrmSearchDevComItem
FrmSearchDevComItem.show
FrmSearchDevComItem.Cmd_Click (0)
 End If
End Sub
...
Private Sub TxtPONo_KeyUp(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyF3 Then
      ...
      If CBoBasedON.ListIndex = 2 Then
            FrmBuySearch.DealingForm = 666
            FrmBuySearch.Index = 20
            FrmBuySearch.Caption = "«·»ÕÀ ⁄‰   «·›« Ê—… «·„»œ∆Ì…"
            FrmBuySearch.show vbModal
        ElseIf CBoBasedON.ListIndex = 3 Then
            Order_no_search.show
            Order_no_search.RetrunType = 73
...
End If
    ...
    Set NewGrid.LblItemsCount = Me.LblItemsCount
       ...
       Set NewGrid.TxtCashCustomerName = Me.TxtCashCustomerName
...
Set NewGrid.DcboEmp = Me.DcboEmp
    ...
    StrSQL = "SELECT * FROM Transactions WHERE Transaction_Type=" & CurrentTransactionType & " and IsNull(chkIsFirstInv,0) = " & IIf(IschkIsFirstInv, 1, 0)
...
Public Sub Retrive(Optional Lngid As Long = 0)
    ...
    TxtTransactionComment.text = IIf(IsNull(rs("TransactionComment").value), "", rs("TransactionComment").value)
     chkIsFirstInv.value = IIf(rs("chkIsFirstInv").value = True, vbChecked, vbUnchecked)
...
XPTxtDiscountVal.text = IIf(IsNull(rs("Trans_Discount").value), "", (rs("Trans_Discount").value))
    ...
    TxtCusBalance.text = IIf(IsNull(rs("CusBalance").value), "", (rs("CusBalance").value))
   ...
   Me.CboPriceType.ListIndex = 0
    ...
    TxtInvoiceNo = GetIncoiceNoByOrder(TxtNoteSerial1.text)
   ...
   End If
    ...
    If Not (RsDetails.EOF Or RsDetails.BOF) Then
        ...
        For Num = 1 To RsDetails.RecordCount
            ...
            ' FG.TextMatrix(Num, FG.ColIndex("LAXIS")) = IIf(IsNull(RsDetails("LAXIS")), "", (RsDetails("LAXIS").value))
            ' FG.TextMatrix(Num, FG.ColIndex("LCYL")) = IIf(IsNull(RsDetails("LCYL")), "", (RsDetails("LCYL").value))
            ' FG.TextMatrix(Num, FG.ColIndex("LSPH")) = IIf(IsNull(RsDetails("LSPH")), "", (RsDetails("LSPH").value))
            ' FG.TextMatrix(Num, FG.ColIndex("RAXIS")) = IIf(IsNull(RsDetails("RAXIS")), "", (RsDetails("RAXIS").value))
            ' FG.TextMatrix(Num, FG.ColIndex("RCYL")) = IIf(IsNull(RsDetails("RCYL")), "", (RsDetails("RCYL").value))
            ' FG.TextMatrix(Num, FG.ColIndex("RSPH")) = IIf(IsNull(RsDetails("RSPH")), "", (RsDetails("RSPH").value))
             ''///////////////
             FG.TextMatrix(Num, FG.ColIndex("OrderLineId")) = IIf(IsNull(RsDetails("LineId")), "", (RsDetails("LineId").value))
...
StrSQL = StrSQL + " left outer join ApprovalData On ScreenName = 'FrmPO3'  and ApprovalData.Transaction_ID"
...
Private Sub SaveData()
    ...
    If Me.TxtModFlg.text <> "R" Then
        ...
        If val(Me.DBCboClientName.BoundText) <> 1 Or val(Me.DBCboClientName.BoundText) <> 2 Then
                ...
                If val(Me.LblFinal) > 0 Then
        
                    Dim MsgRe2 As String
        
                    If CheckCusCredit(val(Me.DBCboClientName.BoundText), val(Me.LblFinal), 0, 0) = False Then
                            Screen.MousePointer = vbDefault
                            Cmd(2).Enabled = True
                       Exit Sub
                     End If
...
Dim TxtNoteSerial1str As String
       ...
       rs("chkIsFirstInv").value = IIf(IschkIsFirstInv, 1, 0)
        
        rs("CusBalance").value = TxtCusBalance.text
...
If XPCboDiscountType.ListIndex = -1 Then
   ...
   rs("Trans_Discount").value = IIf(XPTxtDiscountVal.text = "", Null, val(XPTxtDiscountVal.text))
        ...
        rs("TransactionComment").value = IIf(TxtTransactionComment.text = "", Null, (TxtTransactionComment.text))
        ...
        For RowNum = 1 To FG.rows - 1
            ...
            If FG.TextMatrix(RowNum, FG.ColIndex("Code")) <> "" Then
                ...
                RSTransDetails("LineID").value = RowNum
                    RSTransDetails("QtyFaqtors").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("QtyFaqtors")) = ""), 1, (FG.TextMatrix(RowNum, FG.ColIndex("QtyFaqtors"))))
       ' RSTransDetails("LAXIS").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("LAXIS")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("LAXIS"))))
       ' RSTransDetails("LCYL").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("LCYL")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("LCYL"))))
       ' RSTransDetails("LSPH").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("LSPH")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("LSPH"))))
       ' RSTransDetails("RAXIS").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("RAXIS")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("RAXIS"))))
       ' RSTransDetails("RCYL").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("RCYL")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("RCYL"))))
       ' RSTransDetails("RSPH").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("RSPH")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("RSPH"))))
        ''//////////
        ...
        RSTransDetails("Area").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("Area")) = ""), Null, val(FG.TextMatrix(RowNum, FG.ColIndex("Area"))))
                ...
                RSTransDetails("ItemBalance").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("ItemBalance")) = ""), 1, val(FG.TextMatrix(RowNum, FG.ColIndex("ItemBalance"))))
            ...
            RSTransDetails("OrderLineId").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("OrderLineId")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("OrderLineId"))))
            RSTransDetails("OldID").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("OldID")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("OldID"))))
            RSTransDetails("thickness").value = IIf((FG.TextMatrix(RowNum, FG.ColIndex("thickness")) = ""), Null, (FG.TextMatrix(RowNum, FG.ColIndex("thickness"))))
                ...
                If Not (RsUnitData.BOF Or RsUnitData.EOF) Then
                    ...
                    If RSTransDetails("QtyBySmalltUnit").value <> 0 Then
...
Private Sub PrintReport(Optional ByVal mIndex As Integer = 0)
    On Error GoTo ErrTrap
Dim StrDaeen As String
    
            
            SaveQRCode "transactions", "Transaction_ID", val(XPTxtBillID), TxtNoteSerial1.text, (XPDtbBill.value), _
               (LblFinal.Caption), Picture1, 0, (LblVat.Caption), (LblFinal.Caption)
...
If SystemOptions.IsSalesOrder Then

    If XPTxtBillID.text <> "" Then
        Set SaleReport = New ClsSaleReport
        StrDaeen = GetOmrDaeen
        If mIndex = 0 Then
            SaleReport.ShowPrice XPTxtBillID.text, 96, DcboEmp.text, , , , , , , StrDaeen
        Else
            SaleReport.ShowPrice XPTxtBillID.text, 97, DcboEmp.text, , , , , , , StrDaeen
        End If
    End If
    Exit Sub
End If
   ...
   rs.Resync adAffectCurrent
    ...
    Else
        SaleReport.ShowSallingData XPTxtBillID.text, 124, , 0, 0, , , , , LblFinal.Caption, , val(Me.dcBranch.BoundText), "", , , 0
    End If
    Exit Sub
ErrTrap:
End Sub
...
Set SaleReport = New ClsSaleReport
        SaleReport.ShowPrice XPTxtBillID.text, 16, DcboEmp.text
Exit Sub
   ...
   End If
     ...
     If reportid = 1 Or reportid = 7 Or reportid = 17 Or reportid = 13 Or reportid = 14 Or reportid = 16 Then
        If XPTxtBillID.text <> "" Then
            ...
            GetCustomerAllData val(DBCboClientName.BoundText), , , , , , , , , , , , , , , Nationality, , GID
                    ...
                    SaleReport.ShowSallingDataDetailed XPTxtBillID.text, , , , LblFinal.Caption, TxtSearchCode.text, "", val(0), reportid, Nationality, GID, XPDtbBill.value, AdvPayment, LblInstallCount, LblPrecenValue, LblFirstInstallDate, , , , , Dccurrency.text, val(txt_Currency_rate.text), , , , Balance
    ...
    If reportid = 2 Then
        If XPTxtBillID.text <> "" Then
            ...
            SaleReport.ShowSallingDataDetailed XPTxtBillID.text, , , , LblTotal, TxtSearchCode.text, "", val(0), 2, Nationality, GID, XPDtbBill.value, AdvPayment, LblInstallCount, LblPrecenValue, LblFirstInstallDate, Madyna, hay, Address, val(val(0) + val(LblTotal.Caption)) - val(AdvPayment), Dccurrency.text, val(txt_Currency_rate.text), , val(dcBranch.BoundText), , Balance
    ...
    If ShowType = 1 Then
        If XPTxtBillID.text <> "" Then
            Set SaleReport = New ClsSaleReport
                ...
                SaleReport.ShowSallingDataDetailed XPTxtBillID.text, , , , Round(val(Me.TxtValueAdded.text) + val(0) + val(LblTotal.Caption), SystemOptions.Count_ACCOUNT_digit), TxtSearchCode.text, , val(0), , , , XPDtbBill.value, , , , , , , , , val(Dccurrency.BoundText), , , val(Me.dcBranch.BoundText), PayDes, Balance, X
    ...
    ElseIf ShowType = 40 Then
        ...
        If XPTxtBillID.text <> "" Then
            Set SaleReport = New ClsSaleReport
            SaleReport.ShowSallingDataDetailed2 XPTxtBillID.text, , , , LblFinal.Caption, TxtSearchCode.text, , val(0), val(Dccurrency.BoundText), , val(dcBranch.BoundText), PayDes
...
'            P_Target = PrinterTarget
   ...
   End If
        ...
        If XPTxtBillID.text <> "" Then
            ...
            SaleReport.ShowSallingData XPTxtBillID.text, 0, , val(Me.LblFinal.Caption), 0, , , , , LblFinal.Caption, , val(Me.dcBranch.BoundText), PayDes, , , X
            'ÿ»«⁄… ≈Ì’«· ≈” ·«„ «·‰ﬁœÌ…
...
sql = sql & " Where (dbo.TransactionValueAdded.Transaction_Type = 30) And (dbo.TransactionValueAdded.Transaction_ID = " & val(XPTxtBillID.text) & ")"
rs2.Open sql, Cn, adOpenStatic, adLockOptimistic, adCmdText
If rs2.RecordCount > 0 Then
With Me.VatGrid
rs2.MoveFirst
...
lbl(35).Caption = "Advance Payment"
...
.TextMatrix(0, .ColIndex("Name")) = "Item Name"
.TextMatrix(0, .ColIndex("ItemCase")) = "Case"
.TextMatrix(0, .ColIndex("Count")) = "Qty"
.TextMatrix(0, .ColIndex("Price")) = "Price"
.TextMatrix(0, .ColIndex("DiscountType")) = "Type Discount"
.TextMatrix(0, .ColIndex("DiscountVal")) = "Discount"
