...
sql = " SELECT     SUM(dbo.Transactions.Transaction_NetValue / 1.15 * 15 / 100 ) AS SumVAT"
Else
...
If SystemOptions.PriceWithVAT = True Then
sql = sql & " Having (SUM(dbo.Transactions.Transaction_NetValue / 1.15 * 15 / 100 ) > 0)"
Else
...
sql = " SELECT        SUM(ISNULL(dbo.TblTransactionPayments.value, dbo.Transactions.Transaction_NetValue) ) AS TotalValue"
