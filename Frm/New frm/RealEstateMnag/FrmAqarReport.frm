      ...
      Begin C1SizerLibCtl.C1Elastic C1Elastic1 
         ...
         Begin VB.Frame Fra 
            ...
            Begin VB.CheckBox chkIsShamel 
               ...
               Width           =   1905
            End
            Begin VB.CheckBox chkIsShamelNot 
               Alignment       =   1  'Right Justify
               Caption         =   "«·⁄ﬁÊœ «·€Ì— ‘«„· ›ﬁÿ"
               ForeColor       =   &H000000FF&
               Height          =   255
...
ElseIf Rd(6).value = True Then
    Print_ContractRevenueReport
ElseIf Rd(7).value = True Then
    Print_ContractRevenueReport2
 End If
...
Public Sub Print_ContractRevenueReport()
    ...
    Dim vBranch As Variant, vAqarType As Variant
    Dim vClient As Variant, vOwner As Variant
    Dim vContNo As Variant

    '==========================
    ' 1)  ÕœÌœ «”„ „·› «· ﬁ—Ì—
    '==========================
    StrFileName = App.path & "\REPORTS\REPORTS NEW\" & "rpt_ContractRevenueSummary.rpt"

    If Dir(StrFileName) = "" Then
        MsgBox "„·› «· ﬁ—Ì— €Ì— „ÊÃÊœ: " & StrFileName, vbExclamation + vbMsgBoxRight + vbMsgBoxRtlReading, App.Title
        Exit Sub
    End If
    ...
    ' 2) ﬁ—«¡… ﬁÌ„ «·›·« — „‰ «·›Ê—„
    '==========================
    hasFrom = (Not IsNull(XPDtbFrom.value) And CStr(XPDtbFrom.value) <> "")
    hasTo = (Not IsNull(XPDtpTo.value) And CStr(XPDtpTo.value) <> "")

    If hasFrom Then
        vFrom = CDate(XPDtbFrom.value)
    Else
        vFrom = Null
    End If

    If hasTo Then
        vTo = CDate(XPDtpTo.value)
    Else
        vTo = Null
    End If
    ...
    If Me.DcbBranch.BoundText <> "" Then
        vBranch = val(Me.DcbBranch.BoundText)
    Else
        vBranch = Null
    End If
    ...
    If Me.dcbAqarType.BoundText <> "" Then
        vAqarType = val(Me.dcbAqarType.BoundText)
    Else
        vAqarType = Null
    End If
    ...
    If Me.dbcClient.BoundText <> "" Then
        vClient = val(Me.dbcClient.BoundText)
    Else
        vClient = Null
    End If
    ...
    If Me.dcsupplier.BoundText <> "" Then
        vOwner = val(Me.dcsupplier.BoundText)
    Else
        vOwner = Null
    End If

    ' —ﬁ„ ⁄ﬁœ „⁄Ì‰ («Œ Ì«—Ï ñ ·Ê ⁄‰œﬂ TextBox „À·«)
'    If Me.txtContractNo.text <> "" Then
'        vContNo = val(Me.txtContractNo.text)
'    Else
        vContNo = Null
 '   End If

    ' ‰’ Ê’›Ï ··›· —… ÌŸÂ— ›Ï «· ﬁ—Ì— ·Ê Õ»Ì   ” Œœ„Â
    NoteSerial = ""
    If hasFrom Or hasTo Then
        NoteSerial = "› —… «·≈Ì—«œ«  "
        If hasFrom Then NoteSerial = NoteSerial & "„‰ " & Format$(vFrom, "yyyy/MM/dd") & " "
        If hasTo Then NoteSerial = NoteSerial & "≈·Ï " & Format$(vTo, "yyyy/MM/dd")
    End If
    ...
    With Cmd
        ...
        .CommandText = "Rpt_ContractRevenueSummary"

        ' FromDate
        .Parameters.Append .CreateParameter("@FromDate", adDate, adParamInput)
        .Parameters("@FromDate").value = vFrom

        ' ToDate
        .Parameters.Append .CreateParameter("@ToDate", adDate, adParamInput)
        .Parameters("@ToDate").value = vTo

        ' BranchId
        .Parameters.Append .CreateParameter("@BranchId", adInteger, adParamInput)
        .Parameters("@BranchId").value = vBranch

        ' AqarTypeId
        .Parameters.Append .CreateParameter("@AqarTypeId", adInteger, adParamInput)
        .Parameters("@AqarTypeId").value = vAqarType

        ' ClientId
        .Parameters.Append .CreateParameter("@ClientId", adInteger, adParamInput)
        .Parameters("@ClientId").value = vClient

        ' OwnerId
        .Parameters.Append .CreateParameter("@OwnerId", adInteger, adParamInput)
        .Parameters("@OwnerId").value = vOwner

        ' ContractNo
        .Parameters.Append .CreateParameter("@ContractNo", adInteger, adParamInput)
        .Parameters("@ContractNo").value = vContNo
    End With
    ...
    If SystemOptions.UserInterface = ArabicInterface Then
        ...
        ' ·Ê Õ«»»  »⁄  NoteSerial ﬂ‹ Parameter  «‰Ï:
        ' xReport.ParameterFields(2).AddCurrentValue NoteSerial
        StrReportTitle = ""
    Else
        xReport.ParameterFields(1).AddCurrentValue cCompanyInfo.ArabCompanyName
...
Public Sub Print_ContractRevenueReport2()
    ...
    ' 1)  ÕœÌœ «”„ „·› «· ﬁ—Ì—
    '==========================
    StrFileName = App.path & "\REPORTS\REPORTS NEW\" & "rpt_ContractRevenueSummary2.rpt"
...
'    Else
 ...
 '   End If
    ...
    With Cmd
        .ActiveConnection = Cn
        .CommandType = adCmdStoredProc
        .CommandText = "Rpt_ContractRevenueDetails"
...
If chkIsLegalAffairs.value = vbChecked Then
        StrSQL = StrSQL & " and  dbo.TblOtheExpensAqar.IsLegalAffairs = 1 "
End If
...
'    Dim i As Integer
    ...
    RevWhere = ""
    If hasFrom Then
        RevWhere = RevWhere & " AND d.Installdate >= " & SQLDate(XPDtbFrom.value, True)
    End If
    If hasTo Then
        ' √ﬁ· „‰ «·ÌÊ„ «· «·Ï ⁄‘«‰ Ì‘„· «·ÌÊ„ ﬂ·Â
        RevWhere = RevWhere & " AND d.Installdate < " & SQLDate(DateAdd("d", 1, XPDtpTo.value), True)
    End If
    ...
    '========================
    s = ""
    s = s & "WITH InstallAgg AS ("
    s = s & "    SELECT ContNo,"
    s = s & "           SUM(ISNULL(installValue,0)) AS TotalContract"
    s = s & "    FROM dbo.TblContractInstallments"
    s = s & "    GROUP BY ContNo"
    s = s & "),"
    s = s & " RevenueAgg AS ("
    s = s & "    SELECT i.ContNo,"
    s = s & "           SUM(ISNULL(d.installValue,0)) AS RevenueAmount"
    s = s & "    FROM dbo.tblContractInsAllocationsDetails2 d"
    s = s & "    INNER JOIN dbo.TblContractInstallments i"
    s = s & "        ON d.Installid = i.id"
    s = s & "    WHERE 1 = 1"
    s = s & RevWhere
    s = s & "    GROUP BY i.ContNo"
    s = s & ") "
    s = s & "SELECT "
    s = s & "    c.ContNo,"                                ' —ﬁ„ «·⁄ﬁœ
    s = s & "    c.NoteSerial1,"                          ' —ﬁ„ «·⁄ﬁœ »«·‰Ÿ«„ ·Ê Õ«»»
    s = s & "    ISNULL(ra.RevenueAmount,0) AS RevenueAmount,"   ' „»·€ «·≈Ì—«œ
    s = s & "    ISNULL(ia.TotalContract,0) AS ContractValue,"   ' ﬁÌ„… «·⁄ﬁœ
    s = s & "    ISNULL(ia.TotalContract,0) - ISNULL(ra.RevenueAmount,0) AS Remaining," ' «·„ »ﬁÏ
    s = s & "    cu.CusName, cu.CusNamee,"                ' «”„ «·„” √Ã—
    s = s & "    ow.CusName  AS OwnerName,"
    s = s & "    ow.CusNamee AS OwnerNameE,"
    s = s & "    br.branch_name, br.branch_namee,"        ' «·›—⁄
    s = s & "    aq.aqarname"                             ' «”„ «·⁄ﬁ«— (·Ê Õ«»»  ŸÂ—Â ›Ï «· ﬁ—Ì—)
    s = s & " FROM dbo.TblContract c"
    s = s & " LEFT JOIN InstallAgg ia ON c.ContNo = ia.ContNo"
    s = s & " LEFT JOIN RevenueAgg ra ON c.ContNo = ra.ContNo"
    s = s & " LEFT JOIN dbo.TblCustemers cu ON c.CusID = cu.CusID"
    s = s & " LEFT JOIN dbo.TblAqar aq ON c.Iqar = aq.Aqarid"
    s = s & " LEFT JOIN dbo.TblCustemers ow ON aq.ownerid = ow.CusID"
    s = s & " LEFT JOIN dbo.TblBranchesData br ON aq.BranchId = br.branch_id"
    s = s & " WHERE 1 = 1"
    ...
    ' ›—⁄
    If Me.DcbBranch.BoundText <> "" Then
        StrWhere = StrWhere & " AND aq.BranchId = " & val(Me.DcbBranch.BoundText)
    End If

    ' ‰Ê⁄ «·⁄ﬁ«—
    If Me.dcaqartypeid.BoundText <> "" Then
        StrWhere = StrWhere & " AND aq.aqartypeid = " & val(Me.dcaqartypeid.BoundText)
    End If

    ' «·⁄„Ì· («·„” √Ã—)
    If Me.dbcClient.BoundText <> "" Then
        StrWhere = StrWhere & " AND c.CusID = " & val(Me.dbcClient.BoundText)
    End If

    ' «·„«·ﬂ
    If Me.dcsupplier.BoundText <> "" Then
        StrWhere = StrWhere & " AND aq.ownerid = " & val(Me.dcsupplier.BoundText)
    End If

    ' ·Ê Õ«»» ›· — —ﬁ„ ⁄ﬁœ „⁄Ì‰
'    If Me.txtContractNo.text <> "" Then
'        StrWhere = StrWhere & " AND c.ContNo = " & val(Me.txtContractNo.text)
'    End If

    s = s & StrWhere
    s = s & " ORDER BY c.ContNo"
    ...
    If rs.BOF Or rs.EOF Then
        Msg = "·« ÊÃœ »Ì«‰«  ≈Ì—«œ  Ê«›ﬁ ‘—Êÿ «· ﬁ—Ì—"
        MsgBox Msg, vbExclamation + vbMsgBoxRight + vbMsgBoxRtlReading, App.Title
        Exit Sub
    Else
        rs.MoveFirst
        print_report s       ' ‰›” «·›‰ﬂ‘‰ «··Ï » ” Œœ„Â · ﬁ«—Ì—ﬂ √Ê €Ì¯— «”„Â ·Ê ⁄‰œﬂ Ê«Õœ… Œ«’…
    End If
...
s = s & " WITH PayAgg AS ("
s = s & "     SELECT i.ContNo,"
s = s & "            SUM(ISNULL(d.[value],0)) AS TotalPaid,"
s = s & "            SUM(ISNULL(d.RentValuePayed,0)) AS RentValuePayed,"
s = s & "            SUM(ISNULL(d.CommissionsPayed,0)) AS CommissionsPayed,"
s = s & "            SUM(ISNULL(d.InsurancePayed,0)) AS InsurancePayed,"
s = s & "            SUM(ISNULL(d.WaterPayed,0)) AS WaterPayed,"
s = s & "            SUM(ISNULL(d.ElectricPayed,0)) AS ElectricPayed,"
s = s & "            SUM(ISNULL(d.TelandNetPayed,0)) AS TelandNetPayed,"
s = s & "            SUM(ISNULL(d.OldValuePayed,0)) AS TotalOldValue,"
s = s & "            SUM(ISNULL(d.VATPayed,0)) AS VATPayed"
s = s & "     FROM dbo.TblContractInstallments i"
s = s & "     LEFT JOIN dbo.ContracttBillInstallmentsDone d ON d.istallid = i.id"
s = s & "     GROUP BY i.ContNo"
s = s & " )"
s = s & " SELECT dunit.unitdesc,dunit.unitno,dunit.ACCount,c1.Cus_mobile,"
s = s & "        dunit.Status, tc.TotalContract  RentValue,aq.ownerid,c1.CusID,c1.CusName,c1.CusNamee,"
s = s & "        aq.Aqarid,aq.aqarname,c2.CusName AS ownername,c2.CusNamee AS ownernamee,"
s = s & "        aq.CountryID,aq.cityid,aq.streetname,ut.name AS UnitName,ut.namee AS UnitNamee,"
s = s & "        rs.name AS RentStatusName,rs.namee AS RentStatusNamee,br.branch_name,br.branch_namee,"
s = s & "        dunit.Services,dunit.Water,dunit.electric,dbo.GetLastDateUonit(dunit.Id) AS MaxDate,"
s = s & "        dunit.Id,dbo.GetLastDateUonitRent(dunit.Id) AS lastDate,rs.id AS IdRent,dunit.Comm,"
s = s & "        dunit.InsuranceValue,dunit.customerid,dunit.WCcount,dunit.roomscount,dunit.kithchencount,"
s = s & "        dunit.meterPrice,dunit.haveFurniture,dunit.Floor,dunit.LoungeCount,dunit.ContID,"
s = s & "        tc.NoteSerial1,tc.StrDate,tc.EndDate,tc.FATValue,tc.InsuranceValue,tc.InsuranceValue1,"
s = s & "        tc.InsuranceValueAdd,tc.NetValue,tc.TotalValue,tc.NetValue AS NetValue_2,tc.NetValue AS NetValue_3,"
s = s & "            tc.Servce,"
s = s & "            tc.Electricity,"
s = s & "            tc.CommiValue,"
s = s & "            tc.Water,"
s = s & "            tc.CommiValue,"
s = s & "            tc.CommiValue,"
s = s & "            tc.Phone as Talnet,"

s = s & "        tc.PeriodsID,tc.Periods,aq.aqartypeid,ttype.name AS AqarType,ttype.namee AS AqarTypeE,"
s = s & "        ISNULL(p.TotalPaid,0) AS TotalPaid,ISNULL(p.RentValuePayed,0) AS Paid_Rent,"
s = s & "        ISNULL(p.CommissionsPayed,0) AS Paid_Commissions,ISNULL(p.InsurancePayed,0) AS Paid_Insurance,"
s = s & "        ISNULL(p.WaterPayed,0) AS Paid_Water,ISNULL(p.ElectricPayed,0) AS Paid_Electric,"
s = s & "        ISNULL(p.TelandNetPayed,0) AS Paid_TelandNet,ISNULL(p.TotalOldValue,0) AS Paid_OldValue,"
s = s & "        ISNULL(p.VATPayed,0) AS Paid_VAT,"
s = s & "        CAST(ISNULL(tc.NetValue,0) AS DECIMAL(18,2)) - CAST(ISNULL(p.TotalPaid,0) AS DECIMAL(18,2)) AS RemainingOnNet"
s = s & " FROM dbo.TblAqarDetai dunit"
s = s & " INNER JOIN dbo.TblAqar aq ON dunit.Aqarid = aq.Aqarid"
s = s & " LEFT JOIN dbo.tblAkarType ttype ON aq.aqartypeid = ttype.id"
s = s & " LEFT JOIN dbo.TblContract tc ON dunit.ContID = tc.ContNo"
s = s & " LEFT JOIN dbo.TblBranchesData br ON aq.BranchId = br.branch_id"
s = s & " LEFT JOIN dbo.TblRentStatus rs ON dunit.Status = rs.id"
s = s & " LEFT JOIN dbo.TblAkarUnit ut ON dunit.unittype = ut.id"
s = s & " LEFT JOIN dbo.TblCustemers c1 ON Tc.CusID = c1.CusID"
s = s & " LEFT JOIN dbo.TblCustemers c2 ON aq.ownerid = c2.CusID"
s = s & " LEFT JOIN PayAgg p ON tc.ContNo = p.ContNo"
...
'End If
'If Me.dcaqartypeid.BoundText <> "" Then
'StrWhere = StrWhere & " AND dbo.aq.aqartypeid  = " & val(Me.dcaqartypeid.BoundText)
'End If
'
'If ChkAccredit.value = vbChecked Then
'    StrWhere = StrWhere & " AND IsNull(tc.Accredit,0) = 1 "
'End If
'
'If ChkNotAccredit.value = vbChecked Then
'    StrWhere = StrWhere & " AND IsNull(tc.Accredit,0) = 0 "
'End If
'
'If chkIsShamel.value = vbChecked Then
'    StrWhere = StrWhere & " AND IsNull(tc.IsShamel,0) = 1 "
'End If
'
'If chkIsShamelNot.value = vbChecked Then
'    StrWhere = StrWhere & " AND IsNull(tc.IsShamel,0) = 0 "
'End If
'
'' «·€ˆ ›· —… ContDate «·ﬁœÌ„…
''If Not IsNull(xpdtbfrom.Value) Then
''    StrWhere = StrWhere & " AND tc.ContDate >= " & SQLDate(xpdtbfrom.Value, True)
''End If
''If Not IsNull(XPDtpTo.Value) Then
''    StrWhere = StrWhere & " AND tc.ContDate <= " & SQLDate(XPDtpTo.Value, True)
''End If
'
'Dim hasFrom As Boolean, hasTo As Boolean
'hasFrom = (Not IsNull(xpdtbfrom.value) And CStr(xpdtbfrom.value) <> "")
...
'
'If hasFrom Then
'    StrWhere = StrWhere & " AND (tc.EndDate >= " & SQLDate(xpdtbfrom.value, True) & " OR tc.EndDate IS NULL)"
'End If
'If hasTo Then
'    ' ?? ???? ??? ????? ???? ?????? SQLDate(DateAdd("d",1,XPDtpTo.Value), True) ??? ???? <
'    StrWhere = StrWhere & " AND tc.StrDate <= " & SQLDate(XPDtpTo.value, True)
'End If
'
'
'
'
'If Me.DcbUnitNo.BoundText <> "" Then
'StrWhere = StrWhere & " AND dunit.ID = " & val(Me.DcbUnitNo.BoundText)
'End If
'
'If Me.dcbAqarType.BoundText <> "" Then
'StrWhere = StrWhere & " AND dunit.AqarID = " & val(Me.dcbAqarType.BoundText)
'End If
'
'
'If Me.dcsupplier.BoundText <> "" Then
'StrWhere = StrWhere & " AND aq.ownerid = " & val(dcsupplier.BoundText)
''gr = 2
'End If
'
'If Me.dbcClient.BoundText <> "" Then
'StrWhere = StrWhere & " AND dunit.customerid = " & val(dbcClient.BoundText)
''gr = 2
'End If
'
...
'
'If Me.dcbCityId2.BoundText <> "" Then
'StrWhere = StrWhere & " AND aq.CountryID = " & val(dcbCityId2.BoundText)
''gr = 2
'End If
'
'If Me.dcmCityID.BoundText <> "" Then
'StrWhere = StrWhere & " AND aq.CityID = " & val(dcmCityID.BoundText)
''gr = 2
'End If
'
'
'If Me.DCAkarUnit.BoundText <> "" Then
'StrWhere = StrWhere & " AND  dunit.unittype = " & val(DCAkarUnit.BoundText)
''gr = 2
'End If
'
'
'If Me.dbcAqarStatus.BoundText <> "" Then
'StrWhere = StrWhere & " AND  dunit.Status = " & val(dbcAqarStatus.BoundText)
''gr = 2
'End If
'
'
...
s = s & " )"
s = s & " SELECT dunit.unitdesc,dunit.unitno,dunit.ACCount,c1.Cus_mobile,"
s = s & "        dunit.Status,dunit.RentValue,aq.ownerid,c1.CusID,c1.CusName,c1.CusNamee,"
s = s & "        aq.Aqarid,aq.aqarname,c2.CusName AS ownername,c2.CusNamee AS ownernamee,"
s = s & "        aq.CountryID,aq.cityid,aq.streetname,ut.name AS UnitName,ut.namee AS UnitNamee,"
s = s & "        rs.name AS RentStatusName,rs.namee AS RentStatusNamee,br.branch_name,br.branch_namee,"
s = s & "        dunit.Services,dunit.Water,dunit.electric,dbo.GetLastDateUonit(dunit.Id) AS MaxDate,"
...
s = s & "        tc.NoteSerial1,tc.StrDate,tc.EndDate,tc.FATValue,tc.InsuranceValue,tc.InsuranceValue1,"
s = s & "        tc.InsuranceValueAdd,tc.NetValue,tc.TotalValue,tc.NetValue AS NetValue_2,tc.NetValue AS NetValue_3,"
s = s & "        tc.Servce, tc.Electricity, tc.CommiValue, tc.Water, tc.CommiValue, tc.CommiValue,"
s = s & "        tc.Phone as Talnet,"
s = s & "        tc.PeriodsID,tc.Periods,aq.aqartypeid,ttype.name AS AqarType,ttype.namee AS AqarTypeE,"
s = s & "        ISNULL(p.TotalPaid,0) AS TotalPaid,ISNULL(p.RentValuePayed,0) AS Paid_Rent,"
s = s & "        ISNULL(p.CommissionsPayed,0) AS Paid_Commissions,ISNULL(p.InsurancePayed,0) AS Paid_Insurance,"
...
s = s & " LEFT JOIN dbo.tblAkarType ttype ON aq.aqartypeid = ttype.id"
' —»ÿ «·⁄ﬁœ Õ”» «·⁄·«ﬁ«  «·’ÕÌÕ…:
s = s & " LEFT JOIN dbo.TblContract tc"
s = s & "   ON tc.UnitNo = dunit.Id"           ' «·⁄ﬁœ ? «·ÊÕœ…
s = s & "  AND tc.Iqar   = aq.Aqarid"          ' «·⁄ﬁœ ? «·⁄ﬁ«— ( √ﬂÌœ)
s = s & " LEFT JOIN dbo.TblBranchesData br ON aq.BranchId = br.branch_id"
s = s & " LEFT JOIN dbo.TblRentStatus rs ON dunit.Status = rs.id"
s = s & " LEFT JOIN dbo.TblAkarUnit ut ON dunit.unittype = ut.id"
s = s & " LEFT JOIN dbo.TblCustemers c1 ON tc.CusID = c1.CusID"
...
s = s & " ),"
s = s & " InstAgg AS ("
s = s & "     SELECT ContNo, SUM(ISNULL(installValue,0)) AS TotalValue_FromInstallments"
s = s & "     FROM dbo.TblContractInstallments"
s = s & "     GROUP BY ContNo"
s = s & " )"
s = s & " SELECT dunit.unitdesc,dunit.unitno,dunit.ACCount,c1.Cus_mobile,"
...
s = s & "        dunit.meterPrice,dunit.haveFurniture,dunit.Floor,dunit.LoungeCount,dunit.ContID,"
s = s & "        tc.NoteSerial1,tc.StrDate,tc.EndDate,tc.FATValue,tc.InsuranceValue,tc.InsuranceValue1,"
s = s & "        tc.InsuranceValueAdd,tc.NetValue,"
s = s & "        ia.TotalValue_FromInstallments AS TotalValue,"                ' <-- ?? ???????
s = s & "        tc.NetValue AS NetValue_2,tc.NetValue AS NetValue_3,"
s = s & "        tc.Servce, tc.Electricity, tc.CommiValue, tc.Water, tc.CommiValue, tc.CommiValue,"
s = s & "        tc.Phone AS Talnet,"
s = s & "        tc.PeriodsID,tc.Periods,aq.aqartypeid,ttype.name AS AqarType,ttype.namee AS AqarTypeE,"
s = s & "        ISNULL(p.TotalPaid,0) AS TotalPaid,ISNULL(p.RentValuePayed,0) AS Paid_Rent,"
...
s = s & "        ISNULL(p.TelandNetPayed,0) AS Paid_TelandNet,ISNULL(p.TotalOldValue,0) AS Paid_OldValue,"
s = s & "        ISNULL(p.VATPayed,0) AS Paid_VAT,"
s = s & "        CAST(ISNULL(ia.TotalValue_FromInstallments,0) AS DECIMAL(18,2))"
s = s & "          - CAST(ISNULL(p.TotalPaid,0) AS DECIMAL(18,2)) AS RemainingOnNet" ' <-- ??? ????? ???????
s = s & " FROM dbo.TblAqarDetai dunit"
s = s & " INNER JOIN dbo.TblAqar aq ON dunit.Aqarid = aq.Aqarid"
s = s & " LEFT JOIN dbo.tblAkarType ttype ON aq.aqartypeid = ttype.id"
s = s & " LEFT JOIN dbo.TblContract tc ON tc.UnitNo = dunit.Id AND tc.Iqar = aq.Aqarid"
s = s & " LEFT JOIN dbo.TblBranchesData br ON aq.BranchId = br.branch_id"
s = s & " LEFT JOIN dbo.TblRentStatus rs ON dunit.Status = rs.id"
s = s & " LEFT JOIN dbo.TblAkarUnit ut ON dunit.unittype = ut.id"
s = s & " LEFT JOIN dbo.TblCustemers c1 ON tc.CusID = c1.CusID"
s = s & " LEFT JOIN dbo.TblCustemers c2 ON aq.ownerid = c2.CusID"
s = s & " LEFT JOIN PayAgg p ON tc.ContNo = p.ContNo"
s = s & " LEFT JOIN InstAgg ia ON tc.ContNo = ia.ContNo"
s = s & " WHERE (1 = 1)"
' -- ???? ?????? ??? ?? ????:
' s = s & "   AND aq.BranchId = 2"
' s = s & "   AND (tc.StrDate >= '2018-01-01' OR tc.StrDate IS NULL)"
' s = s & "   AND tc.StrDate <= '2018-12-24'"
' s = s & "   AND dunit.AqarID = 9"
' s = s & "   AND c1.CusID = 1851"
' s = s & " ORDER BY tc.StrDate, tc.ContNo"
...
'
If hasFrom Then
    StrWhere = StrWhere & " AND (tc.StrDate >= " & SQLDate(XPDtbFrom.value, True) & " OR tc.StrDate IS NULL)"
End If
If hasTo Then
    StrWhere = StrWhere & " AND tc.StrDate <= " & SQLDate(XPDtpTo.value, True)
End If
...
If Me.txtStreet.text <> "" Then
    Dim st As String: st = Replace(Me.txtStreet.text, "'", "''")
    StrWhere = StrWhere & " AND aq.streetName LIKE N'%" & st & "%'"
End If

If Me.txtRoomCount.text <> "" Then
    StrWhere = StrWhere & " AND dunit.roomscount = " & val(txtRoomCount.text)
End If
...
' œ·Êﬁ Ì s Ã«Â“ ·· ‰›Ì–
' Set rs = New ADODB.Recordset
' rs.Open s, Conn, adOpenKeyset, adLockReadOnly
...
StrSQL = " SELECT    dbo.TblContract.Accredit, TblContract.noteserial1, TblContract.IsShamel  ,dbo.TblAqarDetai.unitdesc, dbo.TblAqarDetai.unitno, dbo.TblAqarDetai.ACCount, dbo.TblAqarDetai.Status, dbo.TblAqarDetai.RentValue, dbo.TblAqar.ownerid, "
...
If chkIsShamel.value = vbChecked Then
    StrWhere = StrWhere & " AND IsNull(TblContract.IsShamel,0) = 1 "
End If

If chkIsShamelNot.value = vbChecked Then
    StrWhere = StrWhere & " AND IsNull(TblContract.IsShamel,0) = 0 "
End If
...
StrSQL = " SELECT  tblaqar.NOOFYears,  dbo.TblAqar.Aqarid, dbo.TblAqar.aqarname, dbo.TblAqar.CountryID, dbo.TblAqar.cityid, dbo.TblAqar.streetname, dbo.TblBranchesData.branch_name, "
...
Function print_report(Optional NoteSerial As String, Optional Ind As Integer = 0)
    ...
    ElseIf Rd(3) Then
        StrFileName = App.path & "\REPORTS\REPORTS NEW\" & "RsIkarNew.rpt"
    Else
        If Ind = 1 Then
            If SystemOptions.UserInterface = ArabicInterface Then
