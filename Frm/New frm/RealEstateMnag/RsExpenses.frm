...
Private Sub TXTOrDer_no2_Change()
If Me.TxtModFlg = "" Then Exit Sub
If Me.TxtModFlg <> "R" Then
If Me.TXTOrDer_no2.text <> "" Then
RetriveOrder
End If
End If
End Sub
...
If KeyCode = vbKeyF3 Then
 ...
 FrmOrderMaintAqarSearch.m_RetrunType = 11
 FrmOrderMaintAqarSearch.show
End If

End If
End Sub
Public Sub RetriveOrder()
   Dim StrSQL As String
...
StrSQL = " SELECT     dbo.TblOrderMaintenance.ID,TblOrderMaintenance.AqrID,TblAqar.ownerid , "
...
StrSQL = StrSQL & "                      dbo.TblEmployee TblEmployee_1 ON dbo.TblOrderMaintenance.SuperVM = TblEmployee_1.Emp_ID LEFT OUTER JOIN"
  ...
  StrWhere = ""
  StrWhere = " Where dbo.TblOrderMaintenance.ID =" & val(Me.TXTOrDer_no2.text) & ""
    ...
    If rs.BOF Or rs.EOF Then
        ...
        Exit Sub
    Else
        DcbIqara2.BoundText = val(rs!AqrID & "")
        DcbIqara.BoundText = val(rs!AqrID & "")
        dcsupplier.BoundText = val(rs!ownerid & "")
   ...
   Select Case .ColKey(Col)
      Case "unitno"
       ...
       If val(.TextMatrix(Row, .ColIndex("type"))) <> 0 And val(.TextMatrix(Row, .ColIndex("iqarid"))) <> 0 Then
           ...
           Fg_Journal_AfterEdit Row, Fg_Journal.ColIndex("value")
        Else
       If SystemOptions.UserInterface = ArabicInterface Then
...
.TextMatrix(mRow2, .ColIndex("des")) = txtRemarks.text
         ...
         If SystemOptions.OpenAccountAqar = False And Trim(.TextMatrix(mRow2, .ColIndex("AccountCode"))) = "" Then
          .TextMatrix(mRow2, .ColIndex("AccountCode")) = ModAccounts.GetMyAccountCode("TblCustemers", "CusID", val(dcsupplier.BoundText))
         Else
           
            If Trim(.TextMatrix(mRow2, .ColIndex("AccountCode"))) = "" Then
                If IarType <> 0 Then
...
End If
TXTOrDer_no2 = Trim(rs("OrderMaintenanceId").value & "")
...
xx:
 ...
 rs("UnitType").value = IIf(Me.DcbUnitType.text = "", Null, DcbUnitType.BoundText)
  rs("OrderMaintenanceId").value = val(TXTOrDer_no2)
