   ...
   Begin C1SizerLibCtl.C1Elastic C1Elastic1 
      ...
      Begin C1SizerLibCtl.C1Elastic C1Elastic8 
         ...
         Begin MSDataListLib.DataCombo DcBranch 
            ...
            RightToLeft     =   -1  'True
         End
         Begin MSDataListLib.DataCombo cmdActivityTypeId 
            Height          =   315
            Left            =   5760
            TabIndex        =   322
            Top             =   150
...
Private Sub Form_Load()
    ...
    End If
    fill_combo cmdActivityTypeId, StrSQL
    Resize_Form Me
...
Me.Text1(0).Text = IIf(IsNull(rs.Fields("SaleWithOutVat").value), 0, rs.Fields("SaleWithOutVat").value)
  ...
  If Not IsNull(rs("HideakarExitVat2").value) Then
   ...
   cmdActivityTypeId.BoundText = IIf(IsNull(rs("ActivityTypeId").value), "", Trim(rs("ActivityTypeId").value))
...
rs("ValueNew7").value = val(TxtDept(7).Text)
        ...
        rs("ActivityTypeId").value = IIf(cmdActivityTypeId.BoundText = "", Null, val(cmdActivityTypeId.BoundText))
...
ErrTrap:
        ...
        rs.CancelUpdate
End Sub
Function getTotalVATItemsValue(TransType As Integer, _
                               withVAT As Boolean, _
                               Optional Vatyo As Variant = Null, _
                               Optional Remarks As Integer = -1, _
                               Optional PrintReport As Boolean = False, _
                               Optional reporttitle As String, _
                               Optional VstReverse As Integer, _
                               Optional getitemWithVat As Boolean = True) As Double
    Dim sql   As String
    ...
    sql = sql & " Where VatTypes.ID =  " & TransType & "  and  IsNull(Transactions.IsHiddenVat,0) = 0 "

    If val(DcBranch.BoundText) <> 0 Then
        sql = sql & " and  (Transactions.BranchId = " & val(DcBranch.BoundText) & ")"
    End If
    
      If Trim(cmdActivityTypeId.Text) <> "" Then
         sql = sql & "  and     Transactions.BranchId In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
  End If
    ...
    If SystemOptions.PriceWithVAT = False Then
        If withVAT = True Then
            If Vatyo <> 0 Then
            sql = sql & " AND ( IsNull(Vatyo,0) <> 0 ) " 'Or Vatyo Is Not Null
            End If
        Else
            sql = sql & " AND (IsNull(Vatyo,0) = 0 ) " 'Or Vatyo Is Null
        End If
    ...
    If SystemOptions.PriceWithVAT = False Then
        ...
        If IsNull(Vatyo) Then
            sql = sql & " AND  (IsNull(Vatyo,0) = 0 ) " ' (Vatyo Is   Null) "
        ...
        Else
            ...
            If Vatyo = 0 Then
                sql = sql & " AND ( IsNull(Vatyo,0) = " & Vatyo & " ) "
            ElseIf Vatyo = 66 Then
                sql = sql & " AND ( IsNull(Vatyo,0) = " & 0 & " ) "
            Else
                sql = sql & " AND (IsNull(Vatyo,0) =15 or IsNull(Vatyo,0) = " & Vatyo & " ) "
            End If
...
ll:
    ...
    If Vatyo = 66 And SystemOptions.PriceWithVAT Then
        ...
        End If
    ElseIf Vatyo = 66 And Not SystemOptions.PriceWithVAT Then
        If TransType = 21 Or TransType = 9 Then
            sql = sql & " AND IsNull(Transactions.chkTaxExempt,0) = 1"
        End If
    ElseIf Vatyo = 0 And Not SystemOptions.PriceWithVAT And Not withVAT Then
        If TransType = 21 Or TransType = 9 Then
            sql = sql & " AND IsNull(Transactions.chkTaxExempt,0) = 1"
        End If
    Else
    ...
    End If
    '*******************
    If TransType = 22 Or TransType = 5 Or TransType = 9 Or TransType = 21 Then
        If Not getitemWithVat Then
            sql = sql & "    AND (ISNULL(TblItems.ItemWithOutVAT , 0  ) = 1 OR IsNull(Transactions.chkTaxExempt,0) = 1)  " ' «·›· —Â ⁄·Ï «·«’‰«› «·„⁄›ÌÂ ›ﬁÿ
        Else
            sql = sql & "    AND ISNULL(TblItems.ItemWithOutVAT , 0  ) <> 1  " ' «·›· —Â ⁄·Ï «·«’‰«› €Ì— «·„⁄›ÌÂ  ›ﬁÿ
        End If
    End If
    '*******************
...
End If
      ...
      If Trim(cmdActivityTypeId.Text) <> "" Then
         sql = sql & "  and     TblExchangeRequest.BranchId In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
  End If
...
If mType = 1 Or mType = 0 Then
    ...
    sql = sql & "  Where (   not(       NoteID is null   ) and   (dbo.TblHandWages.Vat2    >0))"
          ...
          If Trim(cmdActivityTypeId.Text) <> "" Then
         sql = sql & "  and     TblHandWages.BranchId In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
  End If
    ...
    End If
        ...
        sql = sql & " and  isnull(IsHiddenVat,0) = 0"
      rs2.Open sql, Cn, adOpenStatic, adLockOptimistic, adCmdText
...
ElseIf mType = 2 Or mType = 0 Then
    ...
    End If
              ...
              If Trim(cmdActivityTypeId.Text) <> "" Then
         StrSQL = StrSQL & "  and     t.BranchId In "
        StrSQL = StrSQL & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        StrSQL = StrSQL & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
  End If
        ...
        End If
        StrSQL = StrSQL & " and  isnull(t.IsHiddenVat,0) = 0"
...
End If
      ...
      If Trim(cmdActivityTypeId.Text) <> "" Then
         sql = sql & "  and     TblMinistryContract.BranchId In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
  End If
...
End If
    ...
    If Trim(cmdActivityTypeId.Text) <> "" Then
        sql = sql & "  and     TblTravDueK.BranchId In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
    End If
...
End If
    ...
    If Trim(cmdActivityTypeId.Text) <> "" Then
        sql = sql & "  and     Notes.branch_no In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
    End If
...
End If
    ...
    If Trim(cmdActivityTypeId.Text) <> "" Then
        sql = sql & "  and     DOUBLE_ENTREY_VOUCHERS.branch_id In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
    End If
...
If (docType = 80) Then
    sql = sql & " and   isnull(DOUBLE_ENTREY_VOUCHERS.project_id,0) = 0"
End If
If (docType = 3) Then
...
End If
    ...
    If Trim(cmdActivityTypeId.Text) <> "" Then
        sql = sql & "  and     notes_all.branch_no  In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
    End If
...
End If
    ...
    If Trim(cmdActivityTypeId.Text) <> "" Then
        sql = sql & "  and     TblContract.branch_no  In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
    End If
...
sql = sql & " From dbo.Transactions where 1=1 and  IsNull(Transactions.IsHiddenVat,0) = 0 "
...
'    sql = "     SELECT  sum("
'    sql = sql & "  ("
'    sql = sql & " Transaction_Details.showPrice * Transaction_Details.ShowQty *  ISNULL(dbo.Transactions.Currency_rate, 1)"
'    If TransType = 22 Then
...
sql = "SELECT SUM("
sql = sql & " CASE WHEN Transaction_Details.Vat = 0 THEN 0 ELSE ("
sql = sql & " (Transaction_Details.showPrice * Transaction_Details.ShowQty * ISNULL(dbo.Transactions.Currency_rate, 1)"

' «· Õﬁﬁ „‰ TransType · ÕœÌœ ﬂÌ›Ì… Œ’„ TotalDiscountPerLine
If TransType = 22 Then
    sql = sql & " - ISNULL(TotalDiscountPerLine, 0) * Transaction_Details.ShowQty -"
Else
    sql = sql & " - ISNULL(TotalDiscountPerLine, 0) -"
...
' «· Õﬁﬁ „‰ ‘—Êÿ «·Œ’„ »‰«¡ ⁄·Ï TransType
If TransType = 21 Or TransType = 9 Then
    If SystemOptions.PriceWithVAT = True Then
        sql = sql & " CASE WHEN ItemDiscountType = 1 THEN 0"
        sql = sql & " WHEN ItemDiscountType = 2 THEN (ItemDiscount * ISNULL(dbo.Transactions.Currency_rate, 1))"
        sql = sql & " WHEN ItemDiscountType = 3 THEN ((Transaction_Details.showQty * Transaction_Details.showPrice) * (ItemDiscount / 100) * ISNULL(dbo.Transactions.Currency_rate, 1))"
        sql = sql & " WHEN ItemDiscountType = 4 THEN (Transaction_Details.showQty * Transaction_Details.showPrice * ISNULL(dbo.Transactions.Currency_rate, 1))"
        sql = sql & " ELSE 0 END ) / (1 + " & intervalVat & ")"
    Else
        sql = sql & " CASE WHEN ItemDiscountType = 1 THEN 0"
        sql = sql & " WHEN ItemDiscountType = 2 THEN (ItemDiscount * ISNULL(dbo.Transactions.Currency_rate, 1))"
        sql = sql & " WHEN ItemDiscountType = 3 THEN ((Transaction_Details.showQty * Transaction_Details.showPrice) * (ItemDiscount / 100) * ISNULL(dbo.Transactions.Currency_rate, 1))"
        sql = sql & " WHEN ItemDiscountType = 4 THEN (Transaction_Details.showQty * Transaction_Details.showPrice * ISNULL(dbo.Transactions.Currency_rate, 1))"
        sql = sql & " ELSE 0 END ) * (Vatyo / 100)"
    End If
Else
    sql = sql & " CASE WHEN ItemDiscountType = 1 THEN 0"
    sql = sql & " WHEN ItemDiscountType = 2 THEN (ItemDiscount * ISNULL(dbo.Transactions.Currency_rate, 1))"
...
sql = sql & " ) END ) AS sumVAT"

sql = sql & " FROM VatTypes"
sql = sql & " INNER JOIN Transactions ON VatTypes.ID = Transactions.Transaction_Type"
sql = sql & " LEFT OUTER JOIN TblStore ON Transactions.StoreID = TblStore.StoreID"
sql = sql & " LEFT OUTER JOIN TblBranchesData ON Transactions.BranchId = TblBranchesData.branch_id"
sql = sql & " FULL OUTER JOIN TblUnites"
sql = sql & " RIGHT OUTER JOIN TblItemsUnits ON TblUnites.UnitID = TblItemsUnits.UnitID"
sql = sql & " RIGHT OUTER JOIN TblItems ON TblItemsUnits.JunckID = TblItems.ItemID"
sql = sql & " RIGHT OUTER JOIN Transaction_Details ON TblItems.ItemID = Transaction_Details.Item_ID"
sql = sql & " ON Transactions.Transaction_ID = Transaction_Details.Transaction_ID"
...
Function getManuTotalVATItemsValue(TransType As Integer, Optional VATPer As Integer) As Double
    ...
    If Trim(cmdActivityTypeId.Text) <> "" Then
        sql = sql & "  and     TblVATSettingsDet.BranchId  In "
        sql = sql & " (Select TblBranchesData.branch_id From dbo.TblBranchesData"
        sql = sql & " Where (ActivityTypeId = " & val(cmdActivityTypeId.BoundText) & "))"
    End If
   ...
   ' If SystemOptions.AllItemInVAT = True Then
    ...
    'Else
       ...
       ' If Not SystemOptions.PriceWithVAT Then
            Sales5.Text = getTotalVATItemsValue(21, False, 0, , , , , False) + val(Text1(1).Text)    '„÷«› «·”ﬂ‰
            RSales5.Text = getTotalVATItemsValue(9, False, 0, , , , , False)
       ' End If
  '  End If
    ...
    Purchases5.Text = getTotalVATItemsValue(22, False, , , , , , False)
    RPurchases5.Text = getTotalVATItemsValue(5, False, , , , , , False)
