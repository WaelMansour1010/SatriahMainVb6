...
Select Case Index
    Case 0
        ...
        TxtModFlg.text = "N"
        clear_all Me
        XPTxtBankID.text = CStr(new_id("tblcur", "code", "", True))
...
Rs.Open "[tblcur]", Cn, adOpenStatic, adLockOptimistic, adCmdTable
Me.TxtModFlg.text = "R"
XPBtnMove_Click 2
Exit Sub
ErrTrap:
...
If IsNull(Rs("conv")) = True Then
    txtConv.text = 0
Else
    txtConv.text = Rs("conv")
End If
...
If Me.TxtModFlg.text <> "R" Then
    ...
    Select Case Me.TxtModFlg.text
        Case "N"
            StrSQL = "select * From  tblcur where namee='" & Trim(XPTxtBankName.text) & "'"
    ...
    Rs("conv").Value = Val(txtConv.text)
...
If XPTxtBankID.text <> "" Then
    StrSQL = "select * From notes where CurID=" & Trim(XPTxtBankID.text)
    RsTemp.Open StrSQL, Cn, adOpenStatic, adLockOptimistic, adCmdText
    If Not (RsTemp.EOF Or RsTemp.BOF) Then
        Msg = "·« Ì„ﬂ‰ Õ–› »Ì«‰«  Â–Â «·⁄„·…" & Chr(13)
        Msg = Msg + "Â‰«ﬂ »⁄÷ «·⁄„·Ì«  „— »ÿ… »Â–Â «·⁄„·…"
